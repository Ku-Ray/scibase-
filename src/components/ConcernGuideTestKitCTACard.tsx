...
import type { TestKitProduct } from '@/lib/types'
import type { ConcernGuideTestKitCTAByType } from '@/lib/concern-guide-data'
...
interface Props {
  cta: ConcernGuideTestKitCTAByType
  concernSlug: string
}

/**
 * Pattern B 悩みガイド用 検査キットCTA（typeName単位）。
 * solutionByType[] 末尾・clinicCTAByType の手前に配置。
 * 視覚は青系（border-blue-200 / bg-blue-50/60）で
 * clinicCTAByType（rose系・治療フレーム）と差別化。
 */
export function ConcernGuideTestKitCTACard({ cta, concernSlug }: Props) {
  ...
  return (
    <section
      ref={sectionRef}
      className="mt-6 rounded-2xl border border-blue-200 bg-blue-50/60 px-5 py-5"
    ...
    >
      <div className="flex items-start gap-2.5 mb-2">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-[10px] font-semibold tracking-[0.1em] text-blue-700/80 uppercase">
            測ってから選ぶ
          </p>
          <h4 className="font-semibold text-[15px] sm:text-[16px] text-foreground leading-snug mt-0.5">
            {cta.headline}
          </h4>
        </div>
      </div>
        ...
        {cta.products.map((product) => {
          ...
          return (
            <div
              ...
              className="rounded-xl border border-blue-100 bg-background px-4 py-3.5"
            >
              <div className="flex items-start justify-between gap-3 mb-2 flex-wrap">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
                    <span className="text-[10px] font-semibold bg-amber-500 text-white rounded px-1.5 py-0.5 tracking-[0.08em]">
                      PR
                    </span>
              ...
              <p className="text-[12px] text-muted-foreground leading-[1.9] mb-3">
                {product.whyJa}
              </p>
