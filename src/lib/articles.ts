...
export const articles: Article[] = [
  // ── 0. Pillar #1：30代から始める論文で選ぶ抗老化サプリ完全ガイド ──
  {
    ...
    problemBody: `老化は連続的な変化に見えるが、実際には30歳前後で複数の指標が同時に「下り坂」に切り替わる。

ひとつ目はコラーゲン産生だ。皮膚の真皮（しんぴ：肌の弾力を支える層）にあるコラーゲンの新規産生は、==25歳をピークに毎年約1%ずつ低下==することが複数の研究で示されている。30代の前半までは自覚しづらいが、累積した低下が30代後半から「ハリの低下・たるみ」として可視化される。

ふたつ目はNAD+（細胞のエネルギーを作るのに使われる分子）。Yoshinoらの2020年のRCTを含む複数のヒト試験で、加齢とともに血中・組織内のNAD+濃度が低下することが確認されている。NAD+の低下は、ミトコンドリア（細胞内のエネルギー工場）機能の低下と並行して進む。

みっつ目は成長ホルモンだ。深い睡眠中に分泌される成長ホルモンの量は、==30歳から40歳の10年で約半分==まで減ることが報告されている。これは細胞の修復・代謝・筋肉量・肌の再生すべてに関わる。
...
そして老化の累積は線形ではない。30代で対策を始めた人と40代で始めた人では、50代の時点で見える差が大きくなる。これは**早期対策ほど指数関数的にリターンが大きくなる**、と複数の縦断研究で示されている構造だ。`,
    ...
    scienceBody: `「効く」と書かれたサプリは多いが、その根拠の質は大きく異なる。論文ベースで成分を選ぶときは、まず研究の信頼性ランクを知る必要がある。

最も信頼性が高いのはメタ解析（複数のRCTの結果を統合した分析）。次にRCT（ランダム化比較試験：被験者をランダムに2群に分けて成分群と偽薬群を比べる試験）。その下にコホート研究（観察研究）が来る。査読されていない自社試験や症例報告は、エビデンスとして使えない。

SciBaseでは成分にエビデンスランクを付けて評価している。S（Sランク）は複数のメタ解析で再現性をもって効果が確認されている成分。A（Aランク）は複数のRCTで効果が確認されている成分。B（Bランク）はRCTが存在するが結果がまだ収束していない、または規模が小さい成分。Cはヒト試験が乏しい成分だ。

「効く」とは何かも明確にする必要がある。効果には「何の指標が・どのくらい・どの期間で改善したか」の3つが揃って初めて意味がある。たとえばコラーゲンペプチドのProkschらの2014年RCTでは、==2.5〜5g/日を12週間摂取で皮膚弾力が約28%改善==した、と具体的に数値が出ている。「肌に良い」では情報がまったく足りない。
    ...
    appendixSections: [
      {
        ...
        body: `5領域すべてに同時に手を出すのは現実的ではない。**論文エビデンスの強さ・30代の老化分岐点との合致・コストパフォーマンス**の3軸で、最初に揃えるべきは次の3成分だ（上のカードで一覧化）。

**1位：[オメガ3](/ingredients/omega3)（EPA・DHA／Sランク）** ─ 全身の慢性炎症の抑制・脳機能維持・心血管系の維持で複数のメタ解析が支持する、最も「土台」になる成分。30代から開始する1本目として迷いがない。==1日1,000〜2,000mg（EPA+DHA合計）==で多くの研究が組まれている。月コスト目安は==¥2,000〜3,500==。魚油の酸化を防ぐため、IFOS認証や第三者検査済みの製品を選びたい。

**2位：[ビタミンD](/ingredients/vitamin-d)（Sランク）** ─ 日本人の多くが不足または欠乏域にあり、骨・免疫・気分・筋力の維持に関わる。Charoenngamらの2020年レビューでも、不足の補正だけで複数の指標が改善することが示されている。==1日2,000〜4,000IU==（屋内勤務中心の人）が標準。月コスト目安は==¥500〜1,500==と、最もコスパが高い。

**3位：[コラーゲンペプチド](/ingredients/collagen-peptide)（Aランク）** ─ 30代の見た目の老化分岐点（皮膚弾力・しわ）に最も直接的に届く成分。前述のProkschらのRCTを含む複数のヒト試験で皮膚弾力の改善が報告されている。化粧品メーカー現場の視点からも、外用ケアと内服の組み合わせは合理性が高い。==1日5g・3ヶ月以上==の継続が前提。月コスト目安は==¥3,000〜5,000==。ビタミンCとセットで運用したい。

==3つ揃えても月¥6,000〜10,000==の範囲に収まる。この3成分が「30代から始める抗老化の土台」になる。これより先にNMN・レスベラトロール・美容ドリンクへ手を出すのは順序として遠回りになる。

自分の体・悩み・予算に合わせて優先順位を決めたい場合は、SciBase の[診断ツール（Analyzer）](/analyzer)で7軸（抗老化・肌・脳・ストレス・睡眠・免疫・代謝）から自動で絞り込める。3成分のうちどれを先に入れるかも、悩みの軸で判断できる。`,
      },
      {
...
なお本記事は一般情報の提供を目的としており、個別の医療相談・診断・治療方針の代替ではない。`,
    ...
    solutionBody: `読み終わった直後の「やる気」は2日で消える。ここでの最初の一歩は、==5分で完了する超低負荷==のものに絞りたい。

**ステップ1（今夜・5分）**：自分が一番気になる悩みを1つだけ書き出す。「肌のハリ」「疲労感」「眠りの浅さ」「集中力の低下」など、いま自分に最も刺さるものを選ぶ。複数あれば最も頻度が高いものにする。これが優先順位の起点になる。

**ステップ2（今夜・10分）**：[SciBaseの悩みカテゴリ](/concerns)で書き出した悩みを選び、推奨成分を3つに絞る。あるいは[診断ツール（Analyzer）](/analyzer)で7軸から自動で絞り込む。3成分のうち、まずは1つだけ選ぶ。複数を同時に始めると効果の有無が判別できなくなる。

**ステップ3（今週中）**：iHerbなどで研究使用形態の製品を1つだけ注文する。本記事の各成分CTAから直接遷移できる。第三者検査・GMP認証・適正用量を満たしていれば、ブランドはこだわりすぎなくて良い。
