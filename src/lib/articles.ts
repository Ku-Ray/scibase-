...
ひとつ目はコラーゲン産生だ。皮膚の真皮（しんぴ：肌の弾力を支える層）にあるコラーゲンの新規産生は、25歳をピークに毎年約1%ずつ低下することが複数の研究で示されている。30代の前半までは自覚しづらいが、累積した低下が30代後半から「ハリの低下・たるみ」として可視化される。
...
みっつ目は成長ホルモンだ。深い睡眠中に分泌される成長ホルモンの量は、30歳から40歳の10年で約半分まで減ることが報告されている。これは細胞の修復・代謝・筋肉量・肌の再生すべてに関わる。
...
「効く」とは何かも明確にする必要がある。効果には「何の指標が・どのくらい・どの期間で改善したか」の3つが揃って初めて意味がある。たとえばコラーゲンペプチドのProkschらの2014年RCTでは、2.5〜5g/日を12週間摂取で皮膚弾力が約28%改善した、と具体的に数値が出ている。「肌に良い」では情報がまったく足りない。
    ...
    subsections: [
      {
        ...
        body: `肌の老化対策で論文エビデンスが最も蓄積しているのはコラーゲンペプチド（加水分解コラーゲン）だ。Prokschらの2014年RCT（n=69）では、2.5〜5g/日を12週間摂取で皮膚弾力の有意な改善が報告されている。「胃で分解されるから意味ない」という説は、加水分解型には当てはまらない。低分子ペプチドとして腸管から吸収され、皮膚の細胞に届くことが確認されている。
...
アスタキサンチン（鮭・甲殻類由来の赤い色素）は強力な抗酸化作用を持つ。Tominagaらの2012年RCT（n=65）では、6mg/日・8週間摂取で肌のシワ・水分量・弾力性の改善が報告されている。紫外線による酸化ストレス対策として位置づけが明確だ。`,
      },
      {
        ...
        body: `脳と認知機能の維持で最もエビデンスが強いのはオメガ3、特にDHA（魚油由来の脂肪酸）だ。脳細胞膜の主要構成成分で、加齢に伴う認知機能低下の遅延と関連が複数のメタ解析で示されている。Yurkoらの2022年メタ解析（27RCT・n>5,000）では、長期摂取群で記憶・処理速度の維持が報告されている。1日250〜1,000mg（DHA換算）で多くの研究が組まれている。
...
NMN・NR（NAD+前駆体）は研究注目度が高いが、現時点ではBランク。動物実験では著明な抗老化効果が示されているが、ヒトRCTでは「血中NAD+は上がるが、認知や疲労の指標は有意差が出にくい」のが現状だ。「効果が確定した成分」を期待するならS・Aランクを優先したい。`,
      },
      {
        ...
        body: `代謝・エネルギー領域では、CoQ10（コエンザイムQ10：ミトコンドリアでATP産生に使われる分子）がAランクの中核だ。体内合成は20代をピークに低下し、特にスタチン系の脂質異常症薬を服用している人は低下が加速する（薬名は一般名のみ・併用は医師相談）。100〜200mg/日で疲労感・運動パフォーマンスの維持が複数のRCTで報告されている。
...
ミトコンドリア機能維持の観点では、PQQ・Urolithin Aなど新興成分も注目されているが、ヒト試験はまだ少ない。基礎を埋める順序としては「マグネシウム → CoQ10 → 新興成分」が合理的だ。`,
      },
      {
        ...
        body: `睡眠の質改善で最初に試す価値があるのはグリシン（アミノ酸の一種）だ。Yamadaらの研究では、就寝前3gで深部体温の低下と入眠潜時の短縮が報告されている。副作用が少なく、2週間で変化を体感しやすい点も導入しやすい理由になる。
...
睡眠時間そのものも重要で、Walkerらの研究では6時間以下の慢性睡眠不足が炎症マーカーの上昇と関連すると示されている。サプリは「7時間以上の睡眠を取ったうえでの質改善」として位置づけたい。`,
      },
      {
        ...
        body: `ストレス対策の中核はアシュワガンダ（インド由来のハーブ・KSM-66規格）でSランク。Lopresti・Smith・Chandrasekharらの複数RCTで、コルチゾール（ストレスホルモン）の低下と知覚ストレススコアの改善が一貫して報告されている。300〜600mg/日・8週間で効果が確認されている。
    ...
    appendixSections: [
      {
        ...
        body: `5領域すべてに同時に手を出すのは現実的ではない。論文エビデンスの強さ・30代の老化分岐点との合致・コストパフォーマンスの3軸で、最初に揃えるべきは次の3成分だ。

**最推奨1：オメガ3（EPA・DHA）** ─ Sランク。全身の慢性炎症の抑制・脳機能維持・心血管系の維持で複数のメタ解析が支持する、最も「土台」になる成分だ。30代から開始する1本目として迷いがない。1日1,000〜2,000mg（EPA+DHA合計）で多くの研究が組まれている。月コスト目安は¥2,000〜3,500。魚油の酸化を防ぐため、IFOS認証や第三者検査済みの製品を選びたい。

**2：ビタミンD** ─ Sランク。日本人の多くが不足または欠乏域にあり、骨・免疫・気分・筋力の維持に関わる。Charoenngamらの2020年レビューでも、不足の補正だけで複数の指標が改善することが示されている。1日2,000〜4,000IU（屋内勤務中心の人）が標準。月コスト目安は¥500〜1,500と、最もコスパが高い。

**3：コラーゲンペプチド** ─ Aランク。30代の見た目の老化分岐点（皮膚弾力・しわ）に最も直接的に届く成分。前述のProkschらのRCTを含む複数のヒト試験で皮膚弾力の改善が報告されている。化粧品メーカー現場の視点からも、外用ケアと内服の組み合わせは合理性が高い。1日5g・3ヶ月以上の継続が前提。月コスト目安は¥3,000〜5,000。ビタミンCとセットで運用したい。

**3つ揃えても月¥6,000〜10,000の範囲に収まる。**この3成分が「30代から始める抗老化の土台」になる。これより先に「NMN」「NMN・レスベラトロール」「美容ドリンク」へ手を出すのは順序として遠回りになる。
...
自分の体・悩み・予算に合わせて優先順位を決めたい場合は、SciBase の[診断ツール（Analyzer）](/analyzer)で7軸（抗老化・肌・脳・ストレス・睡眠・免疫・代謝）から自動で絞り込める。3成分のうちどれを先に入れるかも、悩みの軸で判断できる。`,
      },
      {
        ...
        body: `処方薬を服用している場合や妊娠中・授乳中の場合は、サプリの選択基準が大きく変わる。「自然由来だから安全」という思い込みは最もリスクが高い誤解で、抗凝固薬・ピル・抗うつ薬・甲状腺薬・免疫抑制剤・糖尿病薬などとは個別に併用注意点がある。
...
なお本記事は一般情報の提供を目的としており、個別の医療相談・診断・治療方針の代替ではない。`,
    ...
    solutionBody: `読み終わった直後の「やる気」は2日で消える。ここでの最初の一歩は、5分で完了する超低負荷のものに絞りたい。

**ステップ1（今夜・5分）**：自分が一番気になる悩みを1つだけ書き出す。「肌のハリ」「疲労感」「眠りの浅さ」「集中力の低下」など、いま自分に最も刺さるものを選ぶ。複数あれば最も頻度が高いものにする。これが優先順位の起点になる。

**ステップ2（今夜・10分）**：[SciBase の悩みカテゴリ](/concerns)で書き出した悩みを選び、推奨成分を3つに絞る。あるいは[診断ツール（Analyzer）](/analyzer)で7軸から自動で絞り込む。3成分のうち、まずは1つだけ選ぶ。複数を同時に始めると効果の有無が判別できなくなる。
