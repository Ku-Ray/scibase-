...
export const PAIR_SEO: Record<string, { title: string; description: string }> = {
  ...
  'coq10-vs-resveratrol': {
    ...
    description: 'CoQ10（100-300mg/日・ユビキノール推奨）とレスベラトロール（trans型100-500mg/日）を論文で比較。\n\nCoQ10はミトコンドリア電子伝達系の電子運搬体でATP産生+抗酸化（Mortensen 2014 Q-SYMBIO RCTで心不全イベント減・Caso 2007でスタチン誘発筋痛軽減）。レスベラトロールはSIRT1活性化のカロリー制限模倣（Howitz 2003 Nature）。\n\n心不全・スタチン服用・既存ミトコンドリア稼働→CoQ10／SIRT1経路・代謝→レスベラトロールの選び分け、両者経路独立で併用OK。',
  },
  'vitamin-e-vs-coq10': {
  ...
  'nmn-vs-resveratrol': {
    ...
    description: 'NMN（250-500mg/日・NAD+前駆体）とレスベラトロール（trans型100-500mg/日）を論文で比較。\n\nNMNはNAD+補充でサーチュイン活性化基質を供給（Yoshino 2021 Science RCT 250mg/日10週でインスリン感受性改善）。レスベラトロールはSIRT1活性化のカロリー制限模倣（Howitz 2003 Nature）。\n\n基質補充→NMN／活性化シグナル→レスベラトロールで「燃料+点火」の二段補完、月コスト（NMN¥6-12k/レスベラトロール¥1.5-4k）、両者ともヒト寿命延長エビデンスは未確立。',
  },
  'spermidine-vs-coq10': {
  ...
  'panthenol-vs-ceramide': {
    ...
    description: 'パンテノール（D-パンテノール・ビタミンB5前駆体・1-5%）とセラミド外用（0.5-2%・セラミドNP/AP/EOP）を論文で比較。\n\nパンテノールは皮膚内でパントテン酸（CoA前駆体）に変換され角質層の水分保持・上皮再生・修復シグナル（Ebner 2002で皮膚バリア改善）。セラミドは細胞間脂質構成成分で皮膚バリア構築・TEWL抑制（Spada 2018で角層水分量・TEWL改善）。\n\n急性修復・刺激後→パンテノール／中長期バリア・乾燥型敏感肌→セラミドの判断軸、両者併用が王道。',
  },
...
export const PAIR_CUSTOM_FAQS: Record<string, { q: string; a: string }[]> = {
  ...
  'nmn-vs-coq10': [
    {
      q: 'NMNとコエンザイムQ10の違いは？',
      a: '作用層が完全に異なります。NMN（ニコチンアミドモノヌクレオチド）は NAD+ の前駆体で、サーチュイン酵素の活性化・細胞代謝・DNA修復に関与します（Yoshino 2021 RCTで250mg/日10週で血中NAD+上昇確認）。CoQ10（コエンザイムQ10）はミトコンドリア電子伝達系の電子運搬体で、ATP産生・抗酸化に必須。NMN は「NAD+ を増やす」、CoQ10 は「ミトコンドリアでエネルギーを作る」と覚えると整理しやすく、両者は別レイヤーで作用するため補完関係。',
    },
    {
      q: 'NMNとCoQ10、どちらから始めるべき？',
      a: '論文ベースの優先順位は年齢で変わります。\n\n30代前半まではCoQ10から始める方が合理的です：エビデンスが豊富（メタ解析多数）、月コスト¥1,000-3,000、心臓・運動・抗酸化への効果が確立。\n\n30代後半-40代以降は NAD+ が顕著に低下するため（Massudi 2012 で30代以降に組織NAD+の有意低下を報告）、CoQ10にNMNを追加する戦略が王道。NMN単独より、まず基盤としてCoQ10、必要に応じてNMN追加が現実的。',
    },
    {
      q: 'NMNとコエンザイムQ10は併用できる？効果は加算される？',
      a: '併用OK・作用機序が異なるため理論上相補的な関係。NMN は NAD+ を増やしてサーチュイン活性化、CoQ10 はミトコンドリア電子伝達でATP産生を支えるため、別々のステップで老化対策に貢献します。実際のRCTで併用群と単独群を比較した研究は限定的ですが、副作用報告は両者を併用しても増えないため、安全性は高いと考えられます。月コスト¥7,000-15,000の負担を許容できるなら併用が現実的。',
    },
    {
      q: '価格が10倍違う（NMN ¥6-12k vs CoQ10 ¥1-3k）コスパで選ぶなら？',
      a: '純粋なコスパなら CoQ10 が圧倒的。エビデンスは豊富（複数のメタ解析でうっ血性心不全・スタチン誘発筋痛・偏頭痛への効果）、月コスト¥1,000-3,000、ヒトRCT多数。NMN はヒトRCTが2020年以降に蓄積中（10件強）で、血中NAD+上昇は確認されているものの、寿命延長や疾患予防の長期アウトカムは継続研究中。「論文の厚み × コスト」で見ると、まず CoQ10、余裕があれば NMN追加 が論文整合な順序。',
    },
    {
      q: 'NMNを飲むなら何歳から？効果が出るまでは？',
      a: '推奨開始年齢は30代後半-40代以降が論文上の目安。NAD+ は20代で最高値、30代から低下開始、50代で20代の半分前後まで下がる縦断研究が報告されています（Massudi 2012）。効果の指標として血中NAD+上昇は8-12週で確認可能（Yoshino 2021 RCTでは10週で評価）。ただし「老けて見えなくなる」「疲労感が減る」等の体感的効果は個人差が大きく、臨床アウトカムの統一見解はまだ確立していません。8-12週で体感ゼロなら撤退判断も合理的。',
    },
    {
      q: 'CoQ10のユビキノール（還元型）とユビキノン（酸化型）、NMNと組み合わせるならどちらが無難？',
      a: '40代以降の併用ならユビキノール（還元型）を選ぶのが論文上で合理的。\n\n理由：①年齢と変換能力＝López-Lluch 2019 Nutrition等で40代以降は体内のユビキノン→ユビキノール変換能力が低下するため、直接ユビキノールを補給する方が効率的。NMNは30代後半-40代以降のNAD+低下対策（Massudi 2012）に対する補充で、両者とも40代以降の老化対策フォーカスでは併用相性が良い。②吸収率＝Evans 2009 J Funct Foodsでユビキノールはユビキノンより約3〜4倍吸収率が高い報告。空腹時より食後（油を含む食事）でさらに吸収率が上がる脂溶性サプリのため、朝食または夕食時の摂取が現実的。③Kaneka QH原料の信頼性＝Jarrow Formulas Ubiquinol QH-Absorb 200mg・Doctor\'s Best Ubiquinol with Kaneka QH等の主要ブランドが採用する日本鐘淵化学（カネカ）の原料は、ユビキノール市場で世界最大シェアで品質トレーサビリティが高い。\n\n論文ベースの組み合わせ即決：①30代前半まで＝CoQ10ユビキノン100mg/日（NOW Foods/Doctor\'s Best 月¥1,000-1,500）+ NMN追加は任意。②30代後半-40代＝ユビキノール100mg/日（Healthy Origins/Kaneka QH 月¥2,500）+ NMN 250-500mg/日（Tru Niagen NR or Doctor\'s Best NMN）。③50代以降＝ユビキノール200mg/日（Jarrow QH-Absorb 月¥3,200）+ NMN 500-1,000mg/日。④スタチン併用中＝ユビキノール100-200mg/日が王道（Rundek 2004 Arch Neurol RCTで30日で血中CoQ10濃度約40%低下）。NMNと併用の月コストは¥4,000-¥15,000で年齢層・予算で調整。\n\n注意＝ワルファリン服用中はCoQ10がビタミンK類似構造のためINR変動報告あり医師相談、降圧薬服用中は軽度血圧低下相乗のため血圧モニタリング、抗がん剤治療中はNMN・CoQ10とも医師相談前提。',
    },
    {
  ...
  'maca-vs-ashwagandha': [
    ...
    {
      q: 'マカとアシュワガンダは併用できる？',
      a: '理論上は併用OKで、実際にメンズ向け活力サプリやアダプトゲン複合製品で両者は併存しています。作用機序の重なりはHPA軸調整という大枠で似ていますが、コルチゾール経路（アシュワガンダ）と性機能/更年期経路（マカ）でメインターゲットがずれており、相互の効果を阻害する報告はほぼありません。ただし両者ともホルモン依存性疾患（乳がん・前立腺がん・甲状腺機能異常等）、SSRI・抗うつ薬、降圧薬、自己免疫疾患治療薬と併用注意があり、複数の併用注意が重なる場合は単独使用を優先するか医師相談を推奨します。月コストは両方併用で¥3,000〜6,000程度。',
    },
    {
  ...
  'hyaluronic-acid-vs-ceramide': [
    {
      q: 'ヒアルロン酸とセラミドの違いは？',
      a: '保湿の仕組みが完全に異なります。\n\nヒアルロン酸（外用ナトリウム塩0.1〜2%）は自重の最大1,000倍の水分を保持できる糖鎖で、角層〜表皮の含水量を即時に増やしてふっくら感・小じわ・乾燥感に作用します。\n\nセラミド（外用0.5〜2%）は細胞間脂質として皮膚バリアを構築し、水分蒸散（TEWL）を抑制して中長期の乾燥・敏感肌・アトピー素因に作用します（JDS 2017 RCT n=63で経口グルコシルセラミド40mg/日×12週で角層水分量・TEWL改善が報告された）。「水を吸い込むHA（吸水）」vs「水を逃がさないセラミド（バリア）」と覚えると整理しやすく、両者は補完関係。',
    },
    {
    ...
    },
    {
      ...
      a: '両者とも経口エビデンスは外用より限定的ですが、論文蓄積は進んでいます。\n\n経口HA（120〜240mg/日）は腸管で分解されて低分子断片として吸収されるルートが報告されており、JNH 2017 RCT n=60で食物由来HA120mg/日×12週で皮膚水分量改善が示されています。\n\n経口セラミド（米由来グルコシルセラミド40〜80mg/日）はJDS 2017 RCT n=63で角層水分量・TEWL改善が報告されており、外用と経口の併用で相乗効果を狙う設計が機能性表示食品でも普及しています。ただし外用直接アプローチほどの即時効果は期待できず、「外用が主・経口は補助」の位置づけが無難。',
    },
  ],
  'arbutin-vs-kojic-acid': [
    {
  ...
  'ashwagandha-vs-l-theanine': [
    ...
    },
    {
      ...
      a: '併用OK・むしろ二段スタックが現実的。経路が独立（アシュワガンダ＝HPA軸調整／テアニン＝GABA・α波）で相互の効果を阻害せず、慢性ストレス対策（アシュワガンダ）と急性ストレス対策（テアニン）を同時に押さえる多軸アプローチが可能。推奨パターン：朝食後にアシュワガンダ300mg（継続摂取で累積効果）＋ストレスフルなイベント前にテアニン200mg（即時リラックス）、または夜の食後にアシュワガンダ＋就寝1時間前にテアニンの組み合わせ。両者とも依存性・耐性形成のRCTでの強い報告はなく、ベンゾジアゼピン系睡眠薬と比べて翌朝の眠気残りも少ない傾向。',
    },
    {
  ...
  'nmn-vs-nicotinamide-riboside': [
    ...
    },
    {
      ...
      a: '論文ベースのコスパ優位はNR（Tru Niagen 300mg/日 月¥3,500前後）が圧倒的。Conze 2019 Sci Rep RCT n=140 8週で血中NAD+用量依存的上昇が確認され、長期安全性試験データもChromaDex社が蓄積中。NMNは¥6,000-12,000/月とコストが2-4倍高く、ヒトRCTは2020年以降の集中蓄積で歴史が浅め。\n\n「論文の厚み × 月コスト」で見るとまずNRから8-12週試し、効果指標（血中NAD+検査・主観疲労）で判断、不満があれば舌下NMNを追加検討する順序が無難。「ChromaDex特許の独占構造を避けたい」「最新の研究を追いたい」ならNMNから始める選択も合理的で、両者とも血中NAD+上昇は確認されています。',
    },
    {
      q: 'NMNとNRは併用できる？効果は加算される？',
      a: '理論上は併用可能で安全性も高いですが、加算効果は限定的という現状評価。両者とも最終的にNAD+に変換される経路を共有するため、片方を上限まで摂取した後にもう片方を追加しても血中NAD+の追加上昇は頭打ちになる可能性が高い。Damgaard 2023 systematic reviewでも「NMN/NR両者ともNAD+前駆体として有効・両者の臨床アウトカム差は限定的」と評価されています。\n\n現実的には片方を選んで8-12週評価が論文上で合理的で、両方併用しても副作用は増えませんがコストは¥10,000/月超になりやすく、その予算は他のサプリ（CoQ10・オメガ3・マグネシウム）に分散する方が王道。',
    },
    {
      q: 'NMN・NRは何歳から始めるべき？効果指標は？',
      a: '推奨開始年齢は30代後半-40代以降が論文上の目安。Massudi 2012 PLoS One で組織NAD+は20代で最高値、30代から低下開始、50代で20代の半分前後まで下がる縦断研究が報告されています。効果指標は①血中NAD+測定（NR 8-12週で用量依存的上昇）、②主観的疲労・睡眠の質（個人差大）、③臨床アウトカム（インスリン感受性・血圧・歩行速度等のヒトRCTで報告された指標は限定的）。「老けて見えなくなる」「寿命が延びる」等の体感的・長期的効果は現状のヒトRCTでは確立しておらず、Damgaard 2023 systematic reviewが「臨床アウトカム有意差は限定的」と慎重評価。\n\n8-12週で血中NAD+も主観効果も変化ゼロなら撤退判断も合理的。',
    },
    {
      q: 'NMN・NRの副作用と併用注意は？スタチンや抗凝固薬と一緒に飲める？',
      a: 'ヒトRCTで報告された副作用は軽微で（消化器症状・頭痛・潮紅が稀）、致死的有害事象の報告はほぼありません。Conze 2019 Sci Rep ではNR 1,000mg/日まで8週で良好な忍容性が確認されています。\n\n併用注意：①ニコチン酸誘導体（ナイアシン高用量）と同時併用で皮膚潮紅が増える可能性、②糖尿病治療薬との併用ではYoshino 2021でインスリン感受性改善が報告されており血糖モニタリング推奨、③スタチン・抗凝固薬との直接的な薬物相互作用の強い報告は現状ありませんが、長期の心血管イベントへの影響は研究中。\n\n妊娠中・授乳中の安全性は未確立で、避けるべき領域。がん治療中・自己免疫疾患の方は、NAD+はDNA修復にも関わるため担当医に確認が安全。',
    },
  ],
  'melatonin-vs-magnesium-glycinate': [
    {
  ...
  'pantothenic-acid-vs-niacinamide': [
    {
      ...
      a: '作用ターゲットが完全に異なります。\n\nナイアシンアミドはビタミンB3（ニコチンアミド）の単一分子で、外用5%が標準濃度。表皮ケラチノサイトで①NAD+前駆体としてエネルギー代謝、②メラノソーム転送阻害でメラニン拡散抑制、③セラミド合成促進でバリア機能強化、の3経路で作用（Hakozaki 2002 RCT 5%×8週で色素沈着改善・Bissett 2005 で小じわ改善）。\n\nパントテン酸（B5）はCoA前駆体で、外用は主にD-パンテノール（プロビタミンB5）1-5%として配合される。角質層の水分保持・上皮再生・修復シグナル（Ebner 2002 D-パンテノール5%で皮膚バリア機能改善）が中心で、CICA系コスメ（パンテノール+センテラ）にも頻配合。「B3=メラニン抑制+バリア+小じわの多面作用」vs「B5=保湿+修復+鎮静の限定機能」と整理すると分かりやすく、競合関係ではなく完全な補完関係。',
    },
    {
  ...
  'fisetin-vs-quercetin': [
    ...
    },
    {
      ...
      a: '両者とも抗血小板作用とCYP3A4阻害で薬物相互作用に注意が必要。【ワルファリン・抗凝固薬】両者ともINR上昇懸念で併用は医師相談・INRモニタリング必須、手術前2週休薬推奨。【CYP3A4基質薬剤】カルシウム拮抗薬・スタチン（特にシンバスタチン）・タクロリムス・シクロスポリン等は血中濃度上昇懸念があり、処方薬服用中は併用前医師相談が安全側。【その他】①ケルセチン高用量（>1g/日）長期で腎機能影響の報告あり腎疾患既往者注意、②フィセチンは高用量データが限定的、③妊娠中・授乳中は両者ともデータ限定で食事ベース摂取を優先、④胆道閉塞・胆石症既往はCYP3A4阻害で胆汁排泄影響の懸念。\n\nSinclair陣営longevityスタック（NMN+レス+ケルセチン or フィセチン+メトホルミン）は研究者個人の運用で、定期的な血液検査と医師フォローが前提の運用。',
    },
  ],
  'spermidine-vs-resveratrol': [
    {
  ...
  'l-theanine-vs-gaba': [
    ...
    },
    {
      ...
      a: '「直接通過しない」が現在の主流見解で、効果が出る場合は末梢経由の間接作用というのが Factcheck 整合的。【両論】①BBB通過否定派＝Knudsen 1988以来「GABAは極性が高く脂溶性が低いためBBB通過困難」が定説、②末梢経由肯定派＝Boonstra 2015で「腸管神経系・迷走神経経由でHPA軸を修飾する可能性」、Abdou 2006のα波増加報告も末梢経由の可能性。【現状の評価】「経口GABAが脳に直接効く」断定的訴求は論文ベースで成立しない領域で、効果を感じる場合も穏やか・個人差が大きい。PharmaGABA®（ファーマフーズ社・乳酸菌発酵GABA）等の特定形態のヒトRCTは限定的で、Suntheanine®（味の素社・L-テアニン標準化品）の方が大規模RCTが充実しています。',
    },
    {
  ...
  'alpha-lipoic-acid-vs-coq10': [
    ...
    },
    {
      ...
      a: '両薬剤への注意方向が真逆。【αリポ酸×糖尿病薬】ALA高用量（600-1,200mg/日）は糖尿病薬（メトホルミン・SGLT2阻害薬・GLP-1作動薬・DPP-4阻害薬・スルホニル尿素薬・インスリン）併用で低血糖警告必須で、Diabetes Care 推奨血糖モニタリング強化・処方医と用量調整必須。\n\n糖尿病性神経障害適応はチオクト酸（処方医薬品）の領域でサプリ自己判断はNG。【CoQ10×ワルファリン】CoQ10はVK構造類似でVK依存性凝固因子に影響→ワルファリンINR低下＝血栓側のリスク（Engelsen 2002 Thromb Haemost）が報告されており、ワルファリン服用中はINRモニタリング強化が必要。\n\nCoQ10×降圧薬（カルシウム拮抗薬・ACE阻害薬・ARB・β遮断薬）併用は血圧低下増強で起立性低血圧注意。\n\n化学療法中（特にドキソルビシン）はCoQ10の効果文脈で腫瘍内科判断必須（心毒性軽減vs腫瘍細胞保護の議論あり）。両者とも妊娠中・授乳中はエビデンス限定で医師相談、サプリ選択は第三者検査済みブランド推奨。',
    },
    {
  ...
  'resveratrol-vs-pterostilbene': [
    {
      ...
      a: '化学構造と経口バイオアベイラビリティに決定的な差があります。\n\nプテロスチルベンはレスベラトロールのジメチル化体（3,5位ヒドロキシ基がメトキシ基に置換）で。\n\nKapetanovic 2011 Cancer Chemother Pharmacol動物試験で経口バイオアベイラビリティ約4倍（メトキシ基置換でグルクロン酸抱合受けにくい・分子親油性向上）が報告された改良型誘導体。両者ともSIRT1活性化・カロリー制限模倣作用が中心機序（Howitz 2003 Nature 古典／Joseph 2008 J Agric Food Chem 動物試験で同等活性）で経路は共有しますが、用量設計が逆＝レスベラトロール150-500mg/日 vs プテロスチルベン50-250mg/日の少量設計が王道。\n\nヒトRCT本数の絶対差＝レスベラトロールはBrasnyo 2011等で代謝マーカー改善RCT複数本（数十本レベル）vs プテロスチルベンはRiche 2014中心の少数本（数本程度）で蓄積段階が異なります。\n\n「プテロスチルベン=レスベラトロール上位互換」評価は安直で、エビデンス本数・長期安全性の蓄積はレスベラトロール優位、用量効率はプテロスチルベン優位という棲み分け。',
    },
    {
  ...
  'palmitoyl-tripeptide-vs-retinol': [
    ...
    },
    {
      ...
      a: '経路独立で併用OK・刺激リスクほぼゼロの組合せ。【スタック設計】①洗顔→化粧水→ナイアシンアミド5%セラム（バリア補強・色素沈着）→パルミトイルトリペプチドセラム（コラーゲン産生シグナル）→レチノール0.1-0.5%（夜のみ・ターンオーバー）→保湿クリーム→（朝のみ）日焼け止め SPF50+ PA++++が無難な順番。【併用合理性】①経路独立＝ペプチド（シグナル経路・RAR非結合）+ レチノール（RAR/RXR結合・ターンオーバー）で機序重複なし、②刺激ほぼゼロ＝ペプチドはレチノールの赤み・乾燥を増悪させない（ナイアシンアミド・ペプチドはレチノールの刺激緩和補助）、③朝晩分離可能＝朝ペプチド+ナイアシンアミド+VC+日焼け止め、夜ペプチド+レチノール+保湿の優先順位。市販同時配合品（The Ordinary "Buffet" マルチペプチド+HA・La Roche-Posay Retinol B3 Serum・SkinCeuticals A.G.E. Interrupter）も普及しており、の組み立て。月コスト¥6,000-15,000の組合せ範囲、3ヶ月以上の継続が前提。',
    },
  ],
  'centella-asiatica-vs-niacinamide': [
    {
  ...
  'hyaluronic-acid-vs-ceramide-oral': [
    ...
    { q: '経口で本当に肌まで届く？外用と経口どっち優先？', a: '経口と外用は役割分担で「どっちか」ではなく「両方」が現実的。\n\n経口HA・セラミドは腸内代謝後に内因性合成シグナル経由で関与＝直接的に投入HAが皮膚到達ではなくシグナル経路が機序（Iwai 2005/Shigemura 2009のコラーゲンペプチド機序と類似・低分子ペプチド/糖鎖が腸吸収→線維芽細胞シグナル）。\n\nの段階＝①外用が主役（HA外用 0.1-2%が水分保持に即時作用・セラミド外用0.5-2%がバリア機能改善 Spada 2018）+②経口は中長期累積補助（8-12週以上の継続で水分量・TEWL改善が報告）+③朝外用（HA+セラミド+VC15%）+夜外用（セラミド+パンテノール+保湿）+経口（食後）の三層構造の進め方。「経口だけでシミ消える」「飲むだけで肌が変わる」断定NGで生活軸（睡眠・水分・紫外線対策・栄養）と併走が現実解。' },
  ...
  'msm-vs-glucosamine': [
    ...
    { q: '併用注意は？甲殻類アレルギー・抗凝固薬は？', a: 'MSMは🚨稀に消化器症状（軟便・下痢）、🚨抗凝固薬で出血傾向の理論的可能性。\n\nグルコサミンは🚨甲殻類由来（カニ・エビ殻）で甲殻類アレルギー禁忌（植物性発酵由来品で代替可）、🚨糖尿病で血糖変動の議論あり（Anderson 2005で血糖中立的）、🚨抗凝固薬ワルファリンで INR上昇症例報告（モニタリング）、🚨喘息既往はcaution。\n\n両者妊娠中・授乳中データ限定で産科医相談下。' },
  ...
  'panax-ginseng-vs-rhodiola': [
    ...
    { q: '併用注意は？高血圧・抗凝固薬は？', a: '高麗人参は🚨抗凝固薬ワルファリン併用でINR変動報告（出血傾向）、🚨抗血小板薬で出血傾向、🚨MAOI・SSRI併用は精神神経科の領域、🚨高血圧でcaution（理論的血圧上昇）、🚨糖尿病薬で血糖低下増強monitor、🚨ホルモン依存性疾患既往caution、不眠・興奮で夕方以降摂取避ける。\n\nロディオラは🚨双極性障害禁忌（躁転リスク）、🚨MAOI併用NG、SSRI/SNRI注意（セロトニン症候群の理論的可能性）、🚨手術前1-2週間中止検討。' },
    { q: '効果が出るまでと評価は？', a: '【高麗人参】4-12週で評価（Reay 2010 8週・Yennurajalingam 2017 8週）、累積効果型、評価指標は主観的疲労感（FACIT-F）・免疫機能・認知（MMSE/MoCA・反応時間）・性機能（IIEF-5）。\n\n【ロディオラ】1-4週で評価（Phytomedicine 2010 4週・J Sports Med 2017 12週）、即時+累積併存、評価指標は精神疲労・燃え尽きスコア（MBI）・認知パフォーマンス。\n\n12週で効果限定的なら🚨心療内科・内分泌内科の領域。' },
  ],
  'zinc-vs-saw-palmetto': [
  ...
  'red-yeast-rice-vs-policosanol': [
    ...
    { q: '🚨🚨副作用・併用注意は？', a: '🚨🚨紅麹はスタチンと同様の筋肉痛・横紋筋融解症・肝障害リスクで処方薬と同等の注意必須、🚨2024年小林製薬紅麹問題でプベルル酸混入による腎障害多発、🚨🚨スタチン処方薬との重複は危険・他のCYP3A4阻害薬（グレープフルーツ・マクロライド系抗菌薬）併用も筋肉痛リスク、🚨妊娠中・授乳中・肝機能異常・腎機能低下・甲状腺機能低下NG。\n\nポリコサノールは🚨抗凝固薬・抗血小板薬で出血傾向、稀に消化器症状・不眠・頭痛、妊娠中・授乳中NG。' },
  ...
  'lions-mane-vs-rhodiola': [
    { q: 'ライオンズメインとロディオラの違いは？', a: '時間軸が異なる補完関係。\n\nライオンズメイン（500-3,000mg/日）はNGF誘導で12週累積の認知機能・神経再生（Mori 2009 RCT）。\n\nロディオラ（SHR-5 200-600mg/日）は精神疲労・バーンアウト即時改善（Phytomedicine 2010 RCT n=101 SHR-5 170mg/日×4週で計算誤り率改善）。\n\n累積認知機能→ライオンズメイン／急性精神疲労→ロディオラの時間軸補完、両者経路独立で併用OK。' },
    { q: 'ノートロピックスタックの王道は？', a: 'ノートロピックスタック王道は①ライオンズメイン500-3,000mg/日（NGF）+②ロディオラ200-600mg/日（精神疲労）+③L-テアニン100-200mg/日+カフェイン100-200mg+④バコパモニエラ300-600mg/日（記憶累積）+⑤オメガ3 EPA+DHA 1-2g/日（DHA脳膜流動性）+⑥ビタミンB群+ビタミンD+マグネシウムの6-7成分。\n\n月¥6,000-15,000、🚨「天才になる」「IQが上がる」断定NG。' },
  ...
  'coq10-vs-resveratrol': [
    ...
    { q: '併用注意は？ワルファリン・癌治療中は？', a: 'CoQ10は🚨🚨ワルファリン併用でINR低下傾向（Heck 2000）でモニタリング、🚨降圧薬で血圧低下増強、🚨化学療法/PARP阻害薬/がん既往は腫瘍学会推奨医師相談（PMC10177531動物試験データ）、稀に消化器症状。\n\nレスベラトロールは🚨抗凝固薬・抗血小板薬で出血傾向の理論的可能性、🚨CYP3A4阻害でカルバマゼピン等の薬物動態変動、エストロゲン感受性疾患既往caution。' },
    { q: '効果が出るまでと評価は？', a: '4-24週で評価（CoQ10 Mortensen 2014 2年・Caso 2007 30日・Žmitek 2017 12週・レスベラトロール SRT501試験）、累積効果型。\n\n評価指標は心不全症状（NYHA分類・BNP）・主観的疲労感・運動耐容能・スタチン誘発筋痛・血圧・脂質パネル・hs-CRP。\n\n24週で効果限定的なら🚨循環器内科の領域（重度心血管疾患）、生活軸（運動・地中海食・禁煙）が王道の基盤。' },
  ],
  'vitamin-e-vs-coq10': [
    ...
    { q: '併用注意は？ワルファリン・前立腺癌は？', a: 'ビタミンEは🚨🚨抗凝固薬ワルファリン・DOAC併用注意（ビタミンK拮抗作用・INR上昇・出血リスク・Miller 2005メタ）、🚨🚨高用量400IU/日超で心血管リスク上昇（Miller 2005）+前立腺癌リスク上昇（SELECT試験）で50歳以上男性高用量caution、🚨手術前1-2週間中止。\n\nCoQ10は🚨ワルファリン併用でINR低下傾向（Heck 2000）、🚨化学療法中は腫瘍内科判断下。' },
    { q: '効果が出るまでと評価は？', a: '12-24週で評価（ビタミンE Burke 2011 review・CoQ10 Mortensen 2014 2年・Caso 2007 30日・Žmitek 2017 12週）、累積効果型。\n\n評価指標は脂質パネル（LDL/HDL/TG）・hs-CRP・血圧・脂質過酸化指標（MDA・8-OHdG）・心不全症状（NYHA・BNP）・主観的疲労感。\n\n24週で効果限定的なら🚨循環器内科の領域、生活軸（運動・地中海食・禁煙）が王道の基盤。' },
  ],
  'nmn-vs-resveratrol': [
    ...
    { q: 'NMNとNRどっち？レスベラトロールも要る？', a: 'NRはヒトRCT蓄積でNAD+上昇のエビデンスがあり、コスパも優位（Tru Niagen 月¥3,500前後）。NMNはYoshino 2021でインスリン感受性改善のRCTあるが、NRも同等効果が期待される（NAD+上昇経路は共通）。\n\nレスベラトロールはSIRT1活性化経路で機序的補完だがヒト寿命延長エビデンスは未確立、抗酸化・抗炎症補助レイヤー。\n\n抗老化スタックは①NMN or NR+②レスベラトロール+③CoQ10+④ω3+⑤ビタミンD+食事+運動が現実的。' },
    { q: '併用注意は？癌治療中・抗凝固薬は？', a: 'NMNは現時点で重篤副作用報告なし（Yoshino 2021・Mills 2016）、ただし長期データ限定、🚨🚨化学療法中・がん既往はcaution（NAD+補充が腫瘍細胞増殖を支える理論的懸念・腫瘍内科判断下）。\n\nレスベラトロールは🚨抗凝固薬・抗血小板薬で出血傾向、🚨CYP3A4阻害でカルバマゼピン等の薬物動態変動、エストロゲン感受性疾患既往caution（フィトエストロゲン作用）。' },
    { q: '効果が出るまでと評価は？', a: '8-16週で評価（NMN Yoshino 2021 10週・レスベラトロール SRT501試験）、累積効果型。\n\n評価指標は血中NAD+（NMN・専門検査）・インスリン感受性（HOMA-IR）・脂質パネル・血圧・主観的疲労感・運動パフォーマンス。\n\n16週で効果限定的なら用量増量・併用追加（NR/CoQ10/ω3/食事性ポリアミン）・生活軸（運動・地中海食・睡眠・カロリー収支）。' },
  ],
  'spermidine-vs-coq10': [
  ...
  'vitamin-e-vs-omega3': [
    ...
    { q: '併用注意は？ワルファリン・前立腺癌は？', a: 'ビタミンEは🚨🚨抗凝固薬ワルファリン・DOAC併用注意（INR上昇・出血リスク・Miller 2005メタ）、🚨🚨高用量400IU/日超は心血管リスク（Miller 2005）+前立腺癌リスク（SELECT試験 JAMA 2011）で50歳以上男性高用量caution、🚨手術前1-2週間中止。\n\nオメガ3は🚨抗凝固薬で出血傾向、🚨3g/日超は出血傾向（FDA 2000 GRAS）、魚アレルギーは藻油切替。' },
    { q: '効果が出るまでと評価は？', a: '4-12週で評価（ビタミンE Burke 2011 review・オメガ3 REDUCE-IT 5年介入・Mozaffarian 2011メタ）、累積効果型。\n\n評価指標は脂質パネル（LDL/HDL/TG・特にEPA/AA比でω3評価）・hs-CRP・血圧・脂質過酸化指標（MDA・8-OHdG）・主観的疲労感。\n\n12週で効果限定的なら🚨循環器内科の領域、生活軸（運動・地中海食・禁煙）が王道の基盤。' },
  ],
  'nattokinase-vs-vitamin-e': [
  ...
  'panthenol-vs-ceramide': [
    { q: 'パンテノールとセラミドの違いは？', a: '機序が異なる補完関係。\n\nパンテノール（D-パンテノール・ビタミンB5前駆体・1-5%）は皮膚内でパントテン酸（CoA前駆体）に変換され角質層の水分保持・上皮再生・修復シグナル（Ebner 2002 Am J Clin Dermatolで皮膚バリア改善・Yang 2018で創傷治癒促進）。\n\nセラミド外用（0.5-2%・セラミドNP/AP/EOP）は細胞間脂質構成成分で皮膚バリア構築・TEWL抑制（Spada 2018で角層水分量・TEWL改善）。\n\n急性修復→パンテノール／中長期バリア→セラミド。' },
