...
export const PAIR_SEO: Record<string, { title: string; description: string }> = {
  ...
  'l-glutamine-vs-carnosine': {
    ...
    description: 'L-グルタミン（5-15g/日・遊離アミノ酸）とカルノシン（β-アラニル-L-ヒスチジンジペプチド・500-1,000mg/日）の違いを論文で比較。L-グルタミンは腸管上皮細胞の主要エネルギー源・タイトジャンクション構造維持・リーキーガット仮説対策で論文蓄積（Rao 2012 reviewで腸バリア修復・Zhou 2019 Gutメタ解析でIBSの腸透過性改善が報告された）、ストレス・術後・激しい運動後の腸粘膜回復に妥当。カルノシンはβ-アラニルジペプチドで抗糖化（AGEs生成阻害）・抗酸化・神経筋pH緩衝の3経路（Hipkiss 2009 Adv Food Nutr Res reviewで老化関連糖化阻害・Aldini 2011 J Cell Mol Medレビュー）、腸というより全身性の抗老化用途が中心軸。「腸粘膜修復・運動回復→L-グルタミン／抗糖化・抗老化・神経筋pH緩衝→カルノシン」で機序がほぼ独立、併用OKだが目的別の優先順位が異なる。L-グルタミンの肝/腎機能低下時の用量注意、カルノシンの吸収率問題（β-アラニンへの分解）、月コスト、効果が出るまで（L-グルタミン2-4週・カルノシン4-12週）を化粧品メーカー視点で解説。',
  },
  ...
  'beta-alanine-vs-l-citrulline': {
    ...
    description: 'β-アラニン（3.2-6.4g/日・カルノシン前駆体）とL-シトルリン（運動前6-8g・マレート型8-10g）の違いを論文で比較。両者とも運動パフォーマンス補助だがメカニズムが完全に異なります。β-アラニンは筋肉内カルノシン濃度を増やしH+緩衝能を上げて高強度・短時間運動（1-4分の中強度〜高強度）の持久力に作用（Hoffman 2008 Amino Acids RCT n=30で4-6週で持久力改善・Saunders 2017メタ解析）、効果発現は累積型で4-8週の継続摂取が必要、副作用としてピリピリ感（paresthesia）。L-シトルリンは血管内NO産生→血流増加・パンプ感・乳酸クリアランス改善で即効性が特徴（Bailey 2010 J Appl Physiol RCTで6g摂取後NO増加・運動効率改善が報告された）、トレーニング当日〜数日の即時報酬。「持久力（β-アラニン）vs パンプ（シトルリン）」「累積（β-アラニン）vs 即時（シトルリン）」の二軸で補完関係、併用OKで筋トレ前スタックとして実用的。月コスト、フリー型/マレート型シトルリンの違い、運動前のタイミング、効果が出るまで（β-アラニン4-8週・シトルリン当日）を化粧品メーカー視点で解説。',
  },
  ...
  'folic-acid-vs-methyl-folate': {
    ...
    description: '葉酸（モノグルタミン酸型・合成プテロイルモノグルタミン酸・400-800μg/日）と活性型メチル葉酸（5-MTHF・L-メチルフォレート・Quatrefolic®/Metafolin®規格・400-800μg/日）の違いを論文で比較。両者ともDNA合成・神経管閉鎖障害（NTD）予防・メチル化サイクル補酵素として機能するが、代謝経路と推奨される対象層が異なります。モノグルタミン酸型葉酸は厚労省2000年通知の唯一の公的言及対象でMRC 1991 Lancet RCT n=1,817のNTD再発リスク72%減・Czeizel 1992 NEJM RCT n=4,753の初発リスク低減のエビデンス基盤、ただし体内でMTHFR酵素により5-MTHFに変換が必要。MTHFR C677T遺伝子多型（日本人10-15%・ホモ接合体5-7%）保持者はMTHFR酵素活性が低下し合成葉酸→5-MTHF変換効率が下がるため、5-MTHF直接補給が論文上で合理的（Pietrzik 2010 Clin Pharmacokinetで5-MTHFの方が血中濃度上昇が早い・Servy 2014で不育症の5-MTHF優位報告）。「妊活/妊娠中の標準→モノグルタミン酸型（公的言及・コスパ）／MTHFR遺伝子検査陽性・不育症既往・ホモシステイン高値→5-MTHF」の使い分け。「妊娠成立する」「不妊が治る」断定は薬機法/景表法NG、不育症外来は医療領域、抗てんかん薬/メトトレキサート併用注意、葉酸1mg/日超でB12欠乏マスキング、月コスト（モノグルタミン酸型¥300/月・5-MTHF¥1,000-2,000/月）、効果が出るまでを化粧品メーカー視点で解説。',
  },
  'iron-vs-vitamin-c-oral': {
    title: '鉄 vs ビタミンC（経口）｜吸収率2-3倍・併用スタックの論文ベース解説',
    description: '鉄（経口18-60mg/日・グリシン酸キレートFerrochel®/ヘム鉄/フマル酸鉄/硫酸鉄等）とビタミンC（500-1,000mg/日）の関係を論文で比較。両者は「比較」より「併用スタック」が無難な典型ペア。非ヘム鉄（植物性食品・サプリ硫酸鉄/フマル酸鉄/グリシン酸鉄等）は腸管で3価Fe3+から2価Fe2+への還元が吸収律速段階で、ビタミンCはこの還元を促進し吸収率を2-3倍に高める論文蓄積（Hallberg 1989 Am J Clin Nutrで非ヘム鉄吸収率がVC 100mg併用で2-3倍・Cook 2001 Am J Clin Nutrレビューで経典スタック確立）。「鉄欠乏性貧血の女性慢性疲労→鉄+VC同時摂取が論文上」「ヘム鉄（動物性）はVC併用効果が小さい」「鉄サプリ+お茶/コーヒー（タンニン）/カルシウム/PPI/胃切除既往で吸収率低下」の現実的な吸収戦略。鉄遺伝性ヘモクロマトーシス（HFE遺伝子変異 C282Y/H63D・人口0.5%）絶対禁忌・サプリ前のCBC+フェリチン+TIBC+TSAT血液検査必須、男性/閉経後女性はサプリ前検査がより重要、フェリチン>100の非欠乏型は補給回避、VC高用量2,000mg/日以上で腎結石、月コスト、効果が出るまで（鉄欠乏改善8-26週）を化粧品メーカー視点で解説。',
  },
  'methyl-folate-vs-vitamin-b12': {
    title: 'メチル葉酸 vs ビタミンB12｜メチル化サイクルの補完関係を論文で比較',
    description: '活性型メチル葉酸（5-MTHF・400-800μg/日）とビタミンB12（メチルコバラミン500-1,000μg/日・経口高用量推奨）の違いを論文で比較。両者は「比較」より「補完関係」が王道なペア。メチル化サイクル（ホモシステイン→メチオニン変換）で5-MTHFがメチル基をホモシステインに供与・B12がメチオニン合成酵素の補酵素として必須、片方だけでは代謝が回らない構造（Bottiglieri 2005 J Nutrでメチル化サイクル機序確立・Smith 2010 PLOS One VITACOG RCT n=271で葉酸0.8mg+B6 20mg+B12 0.5mg/日×24週で軽度認知障害の脳萎縮率50%以上減少が報告された）。Ageing Research Reviews 2016メタ解析 n=2,398でも葉酸補充で認知機能スコア・ホモシステイン値有意改善（特に高齢者）。一方、葉酸1mg/日超の長期摂取はB12欠乏の血液所見（巨赤芽球性貧血）を隠す可能性あり・神経症状進行リスクで、ベジタリアン/ヴィーガン/PPI長期/胃切除既往はB12不足リスク高。「MTHFR多型陽性→5-MTHF優先／菜食主義者・PPI/胃切除→B12優先／メチル化サイクル最適化→併用」の使い分け、抗てんかん薬/メトトレキサート併用注意、月コスト、効果が出るまで（4-12週）、認知機能/不育症/妊活での位置づけを化粧品メーカー視点で解説。',
  },
}
...
export const PAIR_CUSTOM_FAQS: Record<string, { q: string; a: string }[]> = {
  ...
  's-boulardii-vs-l-rhamnosus-gg': [
    ...
    { q: '効果が出るまでと評価のタイミングは？', a: '【抗生剤関連下痢予防】抗生剤開始と同時開始→終了後2週間継続、効果指標＝下痢発生有無・便回数・性状。【小児急性下痢】摂取24-48時間で症状改善傾向（Hojsak 2010 Cochraneで平均1日短縮）、5日間が王道継続期間。【IBS-D】4-8週で評価、評価指標＝IBS-SSS（IBS Symptom Severity Score）・便回数・腹痛・便性状。\n\n効果限定的なら①株切替（S. boulardii↔LGG）／②マルチストレイン製品試行（VSL#3®・Visbiome®）／③消化器内科受診（IBD/SIBO/セリアック病/慢性膵炎/胆汁酸吸収不良/微小視顕微鏡的大腸炎 鑑別必須）／④食事軸（FODMAP・低乳製品・ストレス管理）／⑤生活軸（睡眠・運動・水分）が次の段階。' },
  ],
  'inulin-vs-psyllium': [
