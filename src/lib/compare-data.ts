export const POPULAR_PAIRS: [string, string][] = [
  ...
  // C2-ε 最終 wave: 13 件追加（2026-05-24）
  ['proteoglycan-salmon', 'hyaluronic-acid-oral'],
  ['elastin-peptide-oral', 'collagen-peptide'],
  ['shilajit-fulvic', 'maca-black-red-yellow'],
  ['pha-gluconolactone', 'glycolic-acid'],
  ['adapalene-01pct-topical', 'tretinoin-topical'],
  ['benzoyl-peroxide-25pct', 'salicylic-acid'],
  ['benzoyl-peroxide-25pct', 'adapalene-01pct-topical'],
  ['d-mannose', 'cranberry-extract'],
  ['riboside-trigonelline', 'nmn'],
  ['dasatinib-senolytic', 'fisetin'],
  ['propolis', 'manuka-honey-mg-rich'],
  ['schisandra', 'milk-thistle'],
  ['vinpocetine', 'citicoline'],

  // ── C2-η Batch 1 パイロット: heart-vascular sparse concern（2026-05-24） ────────────
  ['algal-oil',               'omega3'],

  // ── C2-Disc-β CSV 外発掘 5 件（C2 300 達成セッション）2026-05-24 ────────────
  ['gaba',                    'l-theanine'],
  ['pterostilbene',           'nmn'],
  ['taurine',                 'creatine'],
  ['l-tyrosine',              'rhodiola'],
  ['zinc',                    'vitamin-d'],
  ['vitamin-b3-niacin',       'red-yeast-rice'],       // C2-η Batch 2: A 誤対比（脂質介入機序違い・YMYL重）
  ['potassium',               'magnesium'],            // C2-η Batch 2: C 別カテゴリ（血圧 vs 睡眠/筋・日本人不足ミネラル2大）
]
...
export const DISABLE_QUICK_CTA_PAIRS: Set<string> = new Set([
  ...
  // C2-ε 最終 wave: 用途分担明確 3 件（2026-05-24）
  'adapalene-01pct-topical-vs-tretinoin-topical',
  'benzoyl-peroxide-25pct-vs-salicylic-acid',
  'dasatinib-senolytic-vs-fisetin',

  // ── C2-η Batch 1 パイロット（2026-05-24） ──
  'algal-oil-vs-omega3',                                // 補完関係（vegan/水銀 vs コスパ/EPA・抗凝固薬併用注意）

  // C2-Disc-β CSV 外発掘 5 件（C2 300 達成）2026-05-24
  'gaba-vs-l-theanine',                                 // 補完（即時鎮静 vs 日中集中）
  'pterostilbene-vs-nmn',                               // 補完（SIRT1 直接 vs NAD+ 補充）
  'taurine-vs-creatine',                                // 補完（持続力 vs 瞬発出力）
  'l-tyrosine-vs-rhodiola',                             // 補完（急性ストレス vs 慢性ストレス）
  'zinc-vs-vitamin-d',                                  // 補完（ミネラル酵素 vs ビタミン受容体）
  'vitamin-b3-niacin-vs-red-yeast-rice',                // YMYL重（横紋筋融解・肝障害・スタチン併用禁忌・EU 2022規制）
  'potassium-vs-magnesium',                             // 用途分担（血圧 vs 睡眠/筋）・腎機能/降圧薬境界
])
...
export const PAIR_CATEGORIES: Record<string, string> = {
  ...
  // C2-ε 最終 wave: 13 件追加（2026-05-24）
  'proteoglycan-salmon-vs-hyaluronic-acid-oral': 'antiaging',
  'elastin-peptide-oral-vs-collagen-peptide': 'antiaging',
  'shilajit-fulvic-vs-maca-black-red-yellow': 'men-health',
  'pha-gluconolactone-vs-glycolic-acid': 'skincare',
  'adapalene-01pct-topical-vs-tretinoin-topical': 'skincare',
  'benzoyl-peroxide-25pct-vs-salicylic-acid': 'skincare',
  'benzoyl-peroxide-25pct-vs-adapalene-01pct-topical': 'skincare',
  'd-mannose-vs-cranberry-extract': 'women-health',
  'riboside-trigonelline-vs-nmn': 'longevity',
  'dasatinib-senolytic-vs-fisetin': 'longevity',
  'propolis-vs-manuka-honey-mg-rich': 'immune',
  'schisandra-vs-milk-thistle': 'liver',
  'vinpocetine-vs-citicoline': 'cognitive',

  // ── C2-η Batch 1 パイロット（2026-05-24） ──
  'algal-oil-vs-omega3':                            'cardiovascular',

  // ── C2-Disc-β CSV 外発掘 5 件（C2 300 達成）2026-05-24 ────────────
  'gaba-vs-l-theanine':                             'stress',
  'pterostilbene-vs-nmn':                           'antiaging',
  'taurine-vs-creatine':                            'muscle',
  'l-tyrosine-vs-rhodiola':                         'stress',
  'zinc-vs-vitamin-d':                              'supplement',
  'vitamin-b3-niacin-vs-red-yeast-rice':            'cardiovascular',
  'potassium-vs-magnesium':                         'cardiovascular',
}
...
export const PAIR_SEO: Record<string, { title: string; description: string }> = {
  ...
  // C2-ε 最終 wave: 13 件追加（2026-05-24）
  'proteoglycan-salmon-vs-hyaluronic-acid-oral': {
    title: 'プロテオグリカン vs 経口ヒアルロン酸｜タンパク×多糖の保水比較',
    description: 'プロテオグリカン（鮭軟骨由来5〜10mg/日）と経口ヒアルロン酸（120mg/日）の違いを論文で比較。プロテオグリカンはコアタンパク+多数の糖鎖で構成された複合体、経口ヒアルロン酸は多糖類単体で異なるカテゴリ。Kakizaki 2017 RCT n=21 でPG10mg×8週で皮膚弾力改善、Oe 2017 RCT で経口ヒアルロン酸120mg×12週で皮膚乾燥改善。両者は構造的補完で併用も合理的・月コスト比較・効果が出るまで・どちらから始めるかを化粧品メーカー視点で解説。',
  },
  'elastin-peptide-oral-vs-collagen-peptide': {
    title: 'エラスチンペプチド vs コラーゲンペプチド｜タンパク×タンパクの役割分担',
    description: 'エラスチンペプチド（カツオ由来75mg/日）とコラーゲンペプチド（5g/日）の違いを論文で比較。コラーゲンが真皮の線維束のハリ・引っ張り強度を担い、エラスチンはゴムのような弾力性を担う補完関係。研究本数はコラーゲンが圧倒的（複数メタ解析）、エラスチンは小規模RCT中心。Shiratsuchi 2016 RCT n=24・Hata 2015 細胞実験 vs Proksch 2014 RCT n=69。土台コラーゲン+補助エラスチンの段階的導入を化粧品メーカー視点で解説。',
  },
  'shilajit-fulvic-vs-maca-black-red-yellow': {
    title: 'シラジット vs マカ｜男性活力ミネラル樹脂×根菜アダプトゲン',
    description: 'シラジット（フルボ酸250-500mg/日）とマカ（1500-3000mg/日）の違いを論文で比較。シラジットはヒマラヤ岩盤のミネラル樹脂でフミン酸・フルボ酸・ジベンゾアルファピロン主体、マカはペルー高地の根菜で多糖類・グルコシノレート主体と全く異なるカテゴリ。Pandit 2016 RCT n=75 でシラジット500mg×90日で総テストステロン上昇、Gonzales 2002 RCT n=8 でマカで性欲改善。作用機序は別で併用も合理的・男性活力・スタミナの月コスト・効果が出るまでを解説。',
  },
  'pha-gluconolactone-vs-glycolic-acid': {
    title: 'PHA vs グリコール酸｜AHA代替・刺激性と効果の論文比較',
    description: 'PHA（グルコノラクトン4〜8%外用）とグリコール酸（5〜10%外用）の違いを論文で比較。両者ともヒドロキシ酸の角質剥離成分ですが、PHAは分子量約180Daと大きく緩やかに作用、グリコール酸は76Daと小さく速く浸透する誤対比の関係。Green 2009 直接比較RCT n=40 で12週評価では肌キメ改善が同等・PHAの刺激スコアは有意低。敏感肌・酒さの方はPHA、健常肌・速い効果重視はグリコール酸という使い分けを化粧品メーカー視点で解説。',
  },
  'adapalene-01pct-topical-vs-tretinoin-topical': {
    title: 'アダパレン vs トレチノイン｜外用レチノイドの選び方を論文で比較',
    description: 'アダパレン0.1%とトレチノイン0.025-0.05%の違いを論文で比較。Otberg 2014 Cochrane で両者は病変改善効果が同等・アダパレンは皮膚刺激スコアが有意低、Tu 2010 直接比較RCT n=105 でも刺激性の差を確認。アダパレンは第三世代レチノイドのRAR-β/γ選択性で刺激性が低、トレチノインはRAR全般作用で抗老化の長期エビデンスが豊富。両者とも妊娠中絶対禁忌・日本では医薬品扱いで皮膚科処方が前提。本ページは情報提供のみで、選び方の判断軸を解説。',
  },
  'benzoyl-peroxide-25pct-vs-salicylic-acid': {
    title: '過酸化ベンゾイル vs サリチル酸｜殺菌×角質剥離の作用機序差',
    description: '過酸化ベンゾイル2.5%とサリチル酸（BHA 0.5-2%）の違いを論文で比較。BPOは活性酸素発生による C. acnes 殺菌が主で炎症性ニキビ向け、サリチル酸は油溶性で毛穴内部の角質剥離が主で非炎症性ニキビ・コメド向けの別カテゴリ。Yang 2020 Cochrane n=12,352 でBPO単剤の有効性確立、Zander 1992 でサリチル酸のコメド改善確認。両者は補完的で時間分離併用可・BPOは日本で医薬品扱い・サリチル酸はOTCで購入可。化粧品メーカー視点で解説。',
  },
  'benzoyl-peroxide-25pct-vs-adapalene-01pct-topical': {
    title: 'BPO vs アダパレン｜外用ニキビ薬の補完関係を論文で',
    description: '過酸化ベンゾイル2.5%とアダパレン0.1%の違いを論文で比較。両者は皮膚科ガイドラインで併用が推奨される補完関係で、配合剤（エピデュオゲル）も日本で処方薬として承認されています。Thiboutot 2006 RCT n=653 で配合剤が単剤より病変改善が優れることを確認、Pariser 2014 で6ヶ月維持療法の再発率低下も実証。BPOがC. acnes殺菌・抗炎症、アダパレンが角化正常化・コメド予防の役割分担。両者とも医薬品扱いで皮膚科処方前提・本ページは情報提供のみで解説。',
  },
  'd-mannose-vs-cranberry-extract': {
    title: 'D-マンノース vs クランベリーエキス｜UTI予防の時間軸比較',
    description: 'D-マンノース（2g/日）とクランベリーエキス（PAC換算36mg/日）の違いを論文で比較。Kranjcec 2014 RCT n=308 で両者ともに無治療より再発性UTI予防効果を確認。D-マンノースは大腸菌のFimH線毛に直接結合する物理的ブロック型で即効性、クランベリーのPACはより広域な菌種への作用と長期継続による予防の時間軸タイプ。両者は作用機序が異なるため併用も合理的・ワルファリン併用注意・妊娠中の判断・コスパ比較を解説。',
  },
  'riboside-trigonelline-vs-nmn': {
    title: 'トリゴネリン vs NMN｜コーヒー由来×直接前駆体のNAD+',
    description: 'トリゴネリン（コーヒー由来100-200mg/日）とNMN（250-500mg/日）の違いを論文で比較。両者ともNAD+前駆体ですが経路と研究蓄積が大きく異なる別カテゴリ。トリゴネリンは天然のコーヒー成分で吸収後にニコチン酸代謝に変換される間接前駆体、NMNはニコチンアミドリボシド前駆体として直接NAD+合成に変換される直接前駆体。Membrez 2024 マウス研究でトリゴネリンが筋肉NAD+上昇、Yoshino 2021 RCT n=10 でNMN経口摂取血中NAD+上昇確認。研究本数はNMNが優位、コーヒー摂取と組み合わせる現実的な選び方を解説。',
  },
  'dasatinib-senolytic-vs-fisetin': {
    title: 'ダサチニブ vs フィセチン｜セノリティクスの処方薬×ポリフェノール',
    description: 'ダサチニブ（処方抗がん剤・senolyticプロトコル100mg/日断続）とフィセチン（イチゴ由来100mg/日）の違いを論文で比較。両者ともセノリティクス（老化細胞除去）作用が研究されていますが法的位置が大きく異なる別カテゴリ。Justice 2019 オープン試験 n=14 でD+Q（ダサチニブ+ケルセチン）プロトコルが老化細胞マーカー低下、Yousefzadeh 2018 マウス研究でフィセチン経口で寿命延長・健康スパン改善。ダサチニブは処方薬で医師管理下のみ・フィセチンは健康食品で自己選択可。本ページは情報提供のみで医療判断は処方医にと解説。',
  },
  'propolis-vs-manuka-honey-mg-rich': {
    title: 'プロポリス vs マヌカハニー｜蜂産物の口腔×創傷比較',
    description: 'プロポリス（500-1500mg/日）とマヌカハニー（MGO 250+ 5-10g/日）の違いを論文で比較。両者とも蜂由来の天然防御物質ですが作用範囲が異なる別カテゴリ。プロポリスは植物樹脂主体でフラボノイドが活性指標・歯肉炎・化学療法口内炎で複数RCT・全身免疫サポートに研究蓄積、マヌカハニーはMGO（メチルグリオキサール）含量が活性指標・喉ケア・創傷ケア・上気道感染症補助で研究蓄積。両者とも蜂アレルギー絶対禁忌・1歳未満禁忌・選び方と併用OKを解説。',
  },
  'schisandra-vs-milk-thistle': {
    title: '五味子 vs ミルクシスル｜肝サポートのアダプトゲン×特異的',
    description: '五味子（500-2000mg/日）とミルクシスル（シリマリン200-420mg/日）の違いを論文で比較。両者とも肝サポートのハーブですが対象とエビデンス本数が異なる別カテゴリ。五味子は伝統中医のアダプトゲンで肝代謝・耐久力・ストレス全般に幅広く作用、ミルクシスルはシリマリン（フラボノリグナン）でC型肝炎・肝硬変・薬剤性肝障害に肝特異的エビデンス。Pares 1998 RCT n=200 シリマリン・Saller 2007 メタ解析でミルクシスル優位。慢性肝疾患は医師管理下で・五味子はストレス併発時の選択肢として解説。',
  },
  'vinpocetine-vs-citicoline': {
    title: 'ビンポセチン vs シチコリン｜認知サポートの血流×リン脂質前駆体',
    description: 'ビンポセチン（30mg/日）とシチコリン（500-1000mg/日）の違いを論文で比較。両者とも認知サポート成分ですが作用機序が異なる誤対比の関係。ビンポセチンはツルニチニチソウ由来アルカロイドで脳血流改善作用（PDE1阻害）、シチコリンは細胞膜リン脂質（PC）の前駆体でアセチルコリン合成基質を提供する別経路の作用。Szatmari 2003 メタ解析でビンポセチンの軽度認知改善、Spiers 1996 RCT n=84 でシチコリンの記憶改善。両者は補完的で併用も合理的・効果が出るまで・コスパ比較を解説。',
  },

  // ── C2-η Batch 1 パイロット（2026-05-24） ──
  'algal-oil-vs-omega3': {
    title: '藻類オイル（ヴィーガン DHA/EPA）vs 魚油オメガ3｜水銀ゼロ・コスト 2-3 倍の妥当性を論文で比較',
    description: '藻類オイル（algal oil 微細藻類由来 DHA/EPA・¥3,500-5,500/月）と魚油オメガ3（EPA+DHA・¥1,000-2,500/月）の違いを論文で比較。両者とも DHA/EPA を供給するオメガ3 サプリだが、原料・水銀汚染リスク・DHA/EPA 比・コスト・ヴィーガン適合・エビデンス階層が異なる対立構造（D 上位互換型・魚を経由するか否か）。\n\n魚油オメガ3 は青魚（イワシ・サバ・アンチョビ等）由来の伝統形態で大規模 RCT 蓄積が世界最厚（GISSI-Prevenzione 1999 Lancet RCT n=11,324 心筋梗塞後 EPA+DHA 850mg/日 3.5 年で全死亡 -20%・REDUCE-IT 2019 NEJM Vascepa 高純度 EPA 4g/日で主要心血管イベント -25% 等）、Mozaffarian 2011 Circulation メタ解析で心血管・脳神経・抗炎症介入の主役として位置づけ。一方で水銀・PCB・ダイオキシン汚染リスクが構造的（食物連鎖の上位ほど蓄積）で、IFOS（International Fish Oil Standards）等の第三者認証品で大幅低減できるが完全ゼロではない。藻類オイルは微細藻類（Schizochytrium・Crypthecodinium）培養由来で、魚を経由せず DHA/EPA を直接生産するヴィーガン代替形態。水銀・PCB 汚染リスクが構造的にゼロ（培養タンク内・閉鎖系生産・トレーサビリティ完備）が最大の差別化点。Sanders 2006 Am J Clin Nutr RCT n=79 12 週で algal DHA 200mg/日が血中 DHA 濃度を魚油同等用量と統計的に有意差なく上昇、Bernstein 2012 J Acad Nutr Diet メタ解析でも algal DHA の心血管・脂質（TG 低下）・血圧改善が魚油同等と評価された。Geppert 2006 Br J Nutr RCT 妊娠中女性 200mg/日 algal DHA で安全性・血中 DHA 上昇が確認され、米国産科婦人科学会のヴィーガン妊婦向け推奨でも algal oil が DHA 源として記載される。\n\n「水銀ゼロ・ヴィーガン適合・妊娠中・PCB 等の汚染を回避したい・若い妊娠世代」なら algal-oil（DHA 200-500mg/日・月¥3,500-5,500）が論文上の第一選択、「コスパ最優先・心血管 EPA 重視・大規模 RCT 蓄積を信頼・水銀汚染は IFOS 認証品で許容」なら魚油オメガ3（EPA+DHA 1,000-2,000mg/日・月¥1,000-2,500）が現実的選択。両者は DHA/EPA 比が異なる（algal は DHA 主体・EPA 少なめ／魚油は EPA/DHA 混合）ため、EPA の抗炎症・TG 低下を最大化したいなら魚油優位、DHA 中心の脳・網膜・妊娠中胎児神経発達なら algal でも同等。コスト 2-3 倍プレミアムは水銀ゼロ・ヴィーガン適合・閉鎖系生産トレーサビリティの妥当な対価で、消費者の価値判断次第。注意点：①両者とも抗凝固薬（ワルファリン・DOAC）併用で出血リスクが報告され手術前 1-2 週休薬推奨、②妊娠中授乳中は DHA 推奨摂取量（200-300mg/日）の上限内で安全、③酸化変性に弱く開封後は冷蔵推奨・酸化指標 TOTOX 値が低い製品を選ぶ、④高用量（>3g/日）で消化器症状・魚臭いげっぷ（algal は無臭が長所）、⑤糖尿病薬併用は血糖傾向変動で要モニタリング。化粧品メーカー視点では DHA→皮膚バリア脂質構成→経皮水分蒸散量改善の経路で algal oil・魚油どちらも意義あり、特に魚臭・水銀懸念で継続困難な人ほど algal oil で 6 ヶ月以上の継続性を確保する戦略が現実的。',
  },
  // ── C2-Disc-β CSV 外発掘 5 件（C2 300 達成・stress×2/antiaging/muscle/supplement）2026-05-24 ────────────
  'gaba-vs-l-theanine': {
    title: 'GABA vs L-テアニン｜リラックス系アミノ酸 2 大の経路差と即効性を論文で比較',
    description: 'GABA（γ-アミノ酪酸 100〜300mg/日・¥1,500-3,000/月）と L-テアニン（100〜400mg/日・¥1,500-3,500/月）の違いを論文で比較。両者ともリラックス系アミノ酸サプリだが、作用機序・血液脳関門通過・推奨用途が完全に異なる補完関係。\n\nGABA は中枢神経の主要抑制性神経伝達物質だが血液脳関門通過率は議論があり、サプリは腸-迷走神経経路で作用する仮説、Yamatsu 2014 J Nutr Sci Vitaminol RCT n=40 4 週間で 100mg 就寝前摂取により入眠潜時が有意短縮（p<0.05）・深睡眠増加、Abdou 2012 Biofactors RCT n=63 でストレス負荷後のα波増加・コルチゾール応答軽減を報告。L-テアニンは緑茶由来の遊離アミノ酸で血液脳関門通過確立・覚醒時のリラックス（α波増加・コルチゾール低下・グルタミン酸調節）に作用、Hidese 2021 Nutr Neurosci メタ解析 n=380 で L-テアニン（単独またはカフェイン併用）が注意・記憶・反応速度を有意改善・ストレス応答軽減、Kimura 2007 Biol Psychol RCT n=12 で急性ストレス負荷時の心拍変動・唾液コルチゾールをプラセボ比有意抑制を報告。\n\n「即時のリラックス感・夜の交感神経過剰対策・即効性 30-60 分」なら GABA（100mg 就寝 30-60 分前・月¥1,500-3,000）、「日中の集中力維持＋リラックス・カフェイン緩和・α波増加の研究蓄積」なら L-テアニン（200mg 朝〜午後・月¥1,500-3,500）が論文上の使い分け。両者は経路独立で併用OK・「日中 L-テアニン 200mg（集中＋リラックス）+ 夜 GABA 100mg（即時鎮静）」の二段スタックが現実的。注意点：①GABA は降圧薬・鎮静薬との相互作用の可能性、②L-テアニンは降圧薬併用で血圧低下を強める可能性、③両者とも妊娠・授乳中は安全性データ限定的で医師相談、④日本では機能性表示食品としての届出が両者とも豊富で品質管理された製品から始めるのが安心。化粧品メーカー視点では慢性ストレス→コルチゾール上昇→皮膚バリア低下のループ介入として両者の意義を明示。',
  },
  'pterostilbene-vs-nmn': {
    title: 'プテロスチルベン vs NMN｜抗老化スタックの SIRT1 と NAD+ 補充を論文で比較',
    description: 'プテロスチルベン（ブルーベリー由来スチルベノイド 50〜200mg/日・¥3,000-6,000/月）と NMN（ニコチンアミドモノヌクレオチド 250〜500mg/日・¥6,000-12,000/月）の違いを論文で比較。両者とも抗老化フロンティアサプリだが、作用機序・経路が完全に独立した補完関係。\n\nプテロスチルベンはレスベラトロール骨格にメトキシ基 2 つ追加で生物利用率が約 4 倍に向上した類似体で、SIRT1 活性化・血糖/脂質代謝改善・神経保護効果が中心軸、Riche 2014 J Clin Lipidol RCT n=80 6-8 週で 125mg/日が血圧・LDL コレステロールの有意改善、ただし Riche 2013 J Toxicol RCT 125mg/日 8 週で LDL +17.1mg/dL 上昇報告もあり脂質管理者は医師相談が前提。NMN は NAD+ 前駆体で経路上流（NMN→NAD+ 変換 1 ステップ）からサーチュイン活性化・全身代謝に作用、2024 Tandfonline systematic review メタ解析 n=513 12 週で血中 NAD+ は有意上昇するが「血糖・脂質・その他代謝指標ではほぼ有意差なし」と慎重評価、Yoshino 2021 Science RCT n=25 で閉経後肥満女性の骨格筋インスリン感受性改善を報告。\n\n「SIRT1 活性化＋血圧・脂質代謝改善（LDL リスクあり・要医師相談）」ならプテロスチルベン（125mg/日 食事と一緒・月¥3,000-6,000）、「NAD+ 補充＋サーチュイン活性化＋代謝改善（コスト高）」なら NMN（250-500mg/日・月¥6,000-12,000）が論文上の使い分け。両者は経路独立（SIRT1 直接活性化 vs NAD+ 補充経由）で併用は理論的シナジーあり・「朝 NMN 250mg+食事と一緒にプテロスチルベン 125mg」のスタックが現実的だが月コスト¥9,000-18,000で予算許容範囲内のみ。注意点：①プテロスチルベンの LDL 上昇報告でスタチン・脂質治療中は要医師相談、②NMN は化学療法中・がん既往では PARP 活性・SASP 増強の理論的リスクで併用回避、③抗凝固薬・CYP2C8 基質薬とプテロスチルベンの理論的相互作用、④妊娠・授乳中は両者とも安全性データ限定的、⑤両者とも臨床アウトカム（寿命延長・認知症予防）の確立エビデンスは未確立。化粧品メーカー視点では SIRT1 活性化・NAD+ 補充→皮膚老化抑制の理論的経路は議論中で、外用レチノール・ナイアシンアミドが論文上の確立スキンケアの主軸。',
  },
  'taurine-vs-creatine': {
    title: 'タウリン vs クレアチン｜運動補助 2 大成分の経路差と併用を論文で比較',
    description: 'タウリン（500〜3,000mg/日・¥1,200-2,000/月）とクレアチン（モノハイドレート 3〜5g/日・¥1,500-2,500/月）の違いを論文で比較。両者ともアミノ酸関連の運動補助サプリだが、作用機序・主目的・エビデンス階層が完全に異なる補完関係。\n\nタウリンは心臓・脳・筋肉に高濃度で存在する含硫アミノ酸で、Singh 2023 Science 観察研究で加齢に伴う体内濃度低下（60 歳までに約 80% 低下）と動物の健康寿命延長が示された一方、ヒト長寿効果の RCT は限定的、Waldron 2018 Sports Med メタ解析でタウリン補給が運動持久力・疲労低減を確認・主に有酸素運動・心血管サポート系のエビデンス、Murakami 2015 J Strength Cond Res RCT で運動疲労軽減を報告。クレアチンは ATP 再合成（クレアチンリン酸→ATP）に直接関わるサプリ最強クラスのエビデンス成分、Kreider 2017 JISSN ポジションスタンドで「最も研究蓄積が厚く効果サイズも大きい筋トレ系サプリ」と評価、Forbes 2022 Exp Gerontol メタ解析 n=22,000 で筋力・除脂肪体重・無酸素運動パフォーマンスへの有意効果が確定的、Forbes 2022 Exp Gerontol メタ解析 n=750 で高齢者・睡眠不足時の記憶・処理速度改善も確認。\n\n「高強度短時間出力・筋力・パワー・除脂肪体重」ならクレアチン（5g/日・月¥1,500-2,500）が論文上の第一選択、「有酸素持久力・心血管サポート・抗酸化・浸透圧調節」ならタウリン（1-2g/日・月¥1,200-2,000）が補完的選択。両者は経路独立で併用OK・スタックすると「瞬発出力（クレアチン）＋持続力（タウリン）」の二段構えになる・実際エナジードリンクの多くが両者を同時配合。クレアチンは初期 1-2kg の体重増（筋細胞内水分貯留・副作用ではない）、タウリンは大量摂取で消化器症状・血圧低下に注意。注意点：①リチウム製剤・降圧薬とタウリン併用注意、②腎機能低下のある方はクレアチン医師相談、③心臓病既往はタウリン医師相談、④妊娠・授乳中の長期摂取データは両者とも限定的、⑤エナジードリンク併用での過剰摂取注意（タウリン 3g/日上限目安）。化粧品メーカー視点では運動継続→代謝活性化→皮膚血流改善の経路で両者の間接効果を明示、両者とも美容直接効果の RCT は限定的でスキンケアは外用レチノール・ナイアシンアミドが主軸。',
  },
  'l-tyrosine-vs-rhodiola': {
    title: 'L-チロシン vs ロジオラ｜ストレス下認知のドーパミン前駆体とアダプトゲンを論文で比較',
    description: 'L-チロシン（100〜150mg/kg/日・¥1,500-3,500/月）とロジオラ・ロゼア（SHR-5 規格 170〜200mg/日・¥2,000-4,000/月）の違いを論文で比較。両者ともストレス下認知パフォーマンス改善サプリだが、作用機序・効果の即時性・エビデンス階層が完全に異なる補完関係。\n\nL-チロシンは非必須アミノ酸でドーパミン・ノルアドレナリン・アドレナリン（カテコールアミン）と甲状腺ホルモンの前駆体、Banderet 1989 Brain Res Bull RCT n=20 で軍事訓練下の兵士にチロシン 100mg/kg×4 日が寒冷・低酸素ストレス下の認知パフォーマンス維持、Mahoney 2007 Physiol Behav RCT n=14 で 150mg/kg 単回投与が寒冷暴露下の作業記憶減退を有意抑制を報告。通常時の認知向上効果は限定的だが、強いストレス下（寒冷・睡眠不足）でカテコールアミンを補助。ロジオラ・ロゼアは高地に自生するアダプトゲンで HPA 軸（コルチゾール反応）を調整、Olsson 2009 Phytomedicine RCT n=101 4 週間で SHR-5 170mg/日が夜間勤務医師の精神疲労・認知テスト（計算誤り率）を有意改善、Kasper 2017 J Sports Med RCT n=118 12 週で 200mg/日がバーンアウト症候群患者の燃え尽きスコア・うつ症状・疲労を有意改善を報告。\n\n「急性ストレス時の単回投与・寒冷/低酸素/睡眠不足下の認知維持・即効性 30-60 分」なら L-チロシン（5-7g 単回・ストレス課題 30-60 分前空腹時・月¥1,500-3,500）、「慢性ストレス・バーンアウト・夜間勤務・継続的アダプトゲン作用」ならロジオラ（200mg/日 朝食前 4-12 週継続・月¥2,000-4,000）が論文上の使い分け。両者は経路独立（カテコールアミン前駆体 vs HPA 軸調整）で併用は理論的シナジーあり。注意点：①L-チロシンは MAOI（モノアミンオキシダーゼ阻害薬）絶対禁忌、②甲状腺機能亢進・ADHD 治療薬服用中・統合失調症・双極性障害は両者とも避ける、③ロジオラは抗うつ薬との相互作用可能性・高用量で不安・興奮・不眠が稀、④妊娠・授乳中は両者とも安全性データ限定的、⑤夜の摂取は両者とも避ける（覚醒作用で不眠リスク）。化粧品メーカー視点では慢性ストレス→コルチゾール上昇→皮膚バリア低下のループ介入としてロジオラの意義を明示、L-チロシンは皮膚色素メラニン合成（チロシナーゼ基質）の前駆体でもあるが経口で皮膚色素への影響は限定的。',
  },
  'zinc-vs-vitamin-d': {
    title: '亜鉛 vs ビタミン D｜免疫サプリ 2 大成分の役割分担と併用を論文で比較',
    description: '亜鉛（8〜30mg/日・¥800-1,800/月）とビタミン D（D3 1,000〜4,000IU/日・¥800-1,500/月）の違いを論文で比較。両者とも免疫機能サポートの代表的サプリで COVID-19 補助療法でも併用研究が進んでいるが、作用機序・主目的・エビデンス階層が完全に異なる補完関係。\n\n亜鉛は 200 以上の酵素の補助因子で必須ミネラル、Yee 2020 Dermatology メタ解析 n=1,024 12 週で経口亜鉛補給がニキビスコア有意改善（抗生物質より効果劣るが副作用プロファイル優秀）、Singh 2013 Cochrane メタ解析で亜鉛サプリ 75mg/日以上を症状出現後 24 時間以内に開始すると風邪罹病期間を約 33% 短縮、Prasad 2008 Mol Med コホート n=3,000 で亜鉛欠乏が T 細胞機能・自然免疫低下と関連を報告。ビタミン D は日光で皮膚が合成する脂溶性ビタミンで、Martineau 2017 BMJ メタ解析 n=11,321 で 1,000-4,000IU/日が急性呼吸器感染症リスクを有意低下（OR 0.88・欠乏状態でより顕著）、Bolland 2018 Lancet Diabetes Endocrinol メタ解析 n=81,000 で血中 25(OH)D 50nmol/L 未満群で骨折・転倒リスク低減効果が顕著を報告。\n\n「ニキビ・皮膚改善・免疫の急性風邪対策・テストステロン維持」なら亜鉛（15-30mg/日 食後 30 分・月¥800-1,800）、「呼吸器感染予防・骨密度・筋骨格・抑うつ補助・通年継続」ならビタミン D（D3 2,000IU/日 食事と一緒・月¥800-1,500）が論文上の役割分担。両者は経路独立（亜鉛酵素補因子 vs ビタミン D 受容体核内シグナル）で併用OK・「朝食時 ビタミン D3 2,000IU + 食後 30 分 亜鉛 15mg」のスタックが現実的・COVID-19 補助療法の併用 RCT（Quesada-Gomez 2020 等）もあり論文上の整合性は高い。注意点：①亜鉛 40mg/日超の長期摂取は銅吸収阻害・銅 1-2mg 補給推奨、②ビタミン D 4,000IU/日超は腎結石・サルコイドーシスは医師相談、③亜鉛＋テトラサイクリン・キノロン系抗生物質は 2 時間以上ずらす、④ビタミン D は K2（MK-7）併用で血管石灰化抑制・骨密度改善のシナジー、⑤妊娠中は両者とも推奨量内で安全だが医師相談、⑥日本人は両者とも食事から不足傾向（亜鉛男性 8-11mg・ビタミン D 多くで欠乏）。化粧品メーカー視点では亜鉛＝コラーゲン合成補酵素・皮脂分泌調整、ビタミン D＝免疫調整・抗炎症の経路で両者の皮膚への意義を明示、両者とも経口で皮膚直接効果の RCT は限定的でビタミン C・A との組み合わせが論文上の主軸。',
  },
  'potassium-vs-magnesium': {
    title: 'カリウム vs マグネシウム｜日本人不足ミネラル 2 大成分の役割分担と DASH 食を論文で比較',
    description: 'カリウム（推奨量 男性 3,000・女性 2,600mg/日・実摂取 約 2,300mg/日）とマグネシウム（推奨量 男性 340-380・女性 270-290mg/日・実摂取 約 250mg/日）の違いを論文で比較。両者とも日本人の食事から不足傾向にある必須ミネラルだが、主軸となる効果領域・サプリ用量・薬物相互作用が完全に異なる補完関係。\n\nカリウムは細胞内陽イオン主体で神経伝達・筋収縮・体液浸透圧の維持に必須、DASH 2001 NEJM RCT n=459 11 週で野菜・果物・低脂肪乳の食事パターン（カリウム 4,700mg/日）が収縮期-5.5・拡張期-3.0mmHg 低下（高血圧者では-11.4/-5.5）、Aburto 2013 BMJ メタ解析 n=128,000 でカリウム摂取増（90-120mmol/日相当 3,500-4,700mg）が脳卒中リスク 24% 低下、SSaSS 2021 NEJM RCT n=20,995 5 年で塩化 K 25% 置換塩が脳卒中 14%・主要心血管イベント 13%・全死亡 12% 減（中国農村部・腎機能正常者）を報告。マグネシウムは 300 以上の酵素反応に関わる必須ミネラル、Abbasi 2012 J Res Med Sci RCT n=46 高齢者 500mg/日 8 週で PSQI（ピッツバーグ睡眠質問票）改善、Zhang 2016 Hypertension メタ解析 n=2,028 で Mg 補給が収縮期-2.0・拡張期-1.78mmHg 低下、Veronese 2016 European J Clin Nutr メタ解析で欠乏状態の人へのインスリン抵抗性・CRP（炎症指標）改善を報告。\n\n「血圧改善・脳卒中予防・むくみ対策・減塩補助」ならカリウム（食事優先・野菜・果物・海藻・芋類で 3,000-4,700mg/日・サプリは FDA 規制で 99mg/錠上限）、「睡眠の質改善・筋疲労軽減・偏頭痛予防・脚のむずむず・便秘」ならマグネシウム（グリシン酸 200-400mg/日 月¥1,400-2,000）が論文上の役割分担。両者は経路独立（カリウム＝細胞外液量・血管平滑筋緊張調整 vs マグネシウム＝GABA 系・NMDA 受容体・神経筋接合部リラックス）で併用 OK・「朝食時 K リッチ食材＋夜マグネシウムグリシン酸 200mg」のスタックが現実的。注意点：①腎機能低下・降圧薬（ARB/ACE 阻害薬）・カリウム保持性利尿薬（スピロノラクトン）服用中は高 K 血症リスクで医師相談必須、②マグネシウム＋テトラサイクリン・フルオロキノロン系抗生物質・ビスホスホネート・甲状腺薬は 2-4 時間ずらす、③妊娠中は両者とも推奨量内で安全だが医師相談、④日本人は両者とも食事から不足傾向で食事改善が第一選択、⑤サプリの位置づけは「食事で不足分の補助」が論文上の主軸。化粧品メーカー視点では慢性ストレス・睡眠不足→コルチゾール上昇→皮膚バリア低下のループでマグネシウムの意義を明示、カリウムは食事改善経由で「むくみ・夕方の顔のはれ」の自覚的改善を補助。',
  },
  'vitamin-b3-niacin-vs-red-yeast-rice': {
    title: 'ナイアシン vs 紅麹｜「天然サプリ」vs「天然スタチン」の致命的ギャップを論文で比較',
    description: 'ナイアシン高用量（ニコチン酸 500〜2,000mg/日・¥800-1,500/月）と紅麹（モナコリンK 3〜10mg/日・¥1,800-3,500/月）の違いを論文で比較。両者とも「サプリで脂質改善」のイメージで並びがちだが、薬理学的属性が完全に異なる致命的ギャップを持つ補完関係。\n\nナイアシンは水溶性ビタミン B3 の高用量形態で、Coronary Drug Project 1986 J Am Coll Cardiol RCT n=8,341 15 年追跡で 3g/日が心筋梗塞既往男性の総死亡率 11% 低下、JAMA 2000 メタ解析 n=30,000 で 1,500-3,000mg/日が HDL+20-25%・LDL-15-20%・トリグリセリド-25-35% の脂質改善、AIM-HIGH 2011 NEJM RCT n=3,414 で徐放性 1,500-2,000mg/日のスタチン併用は心血管イベントの追加抑制効果を示せず試験中止、HPS2-THRIVE 2014 NEJM RCT n=25,673 で重篤有害事象（糖尿病・出血・感染）増加を報告。紅麹（モナコリンK）は薬理学的にロバスタチン（処方薬スタチン）と完全に同一物質で、Becker 2009 Ann Intern Med RCT n=62 でモナコリンK 6mg/日 24 週がスタチン不耐患者の LDL -35mg/dL 有意低下、CCSPS 2008 Am J Cardiol RCT n=4,870 4 年で Xuezhikang 600mg×2/日が心血管死-31%・冠動脈疾患-45% 低下、Cochrane 2019 systematic review n=8,535 が LDL 低下を確立する一方シトリニン混入・モナコリンK 含有量ばらつきを品質課題と整理。\n\n「水溶性ビタミン B3 の高用量・フラッシュ反応許容・肝機能定期確認・医師管理下」ならナイアシン、「サプリ感覚での自己購入はスタチン薬理リスク」が紅麹の現実評価。EU 2022 規制（モナコリンK 3mg/日超の販売規制）と 2024 年小林製薬紅麹事案（プベルル酸混入で 5 名死亡・約 240 名入院）以降、紅麹サプリの自己購入・自己判断使用は極めて慎重に判断すべきです。両者とも単独で処方薬同等の作用・副作用プロファイルを持ち、スタチン併用は両者とも横紋筋融解症・肝障害リスクで絶対禁忌。「天然成分だから安全」発想は危険で、両者とも医師管理下の使用が安全側。グレープフルーツ・CYP3A4 阻害薬・抗凝固薬・甲状腺薬との相互作用、妊娠中・授乳中・肝疾患・腎機能障害の絶対禁忌、紅麹伝統食品（豆腐よう・腐乳）とサプリ濃縮形態の区別、スタチン代替検討時の主治医相談前提を化粧品メーカー視点で解説。',
  },
}
...
export const PAIR_CUSTOM_FAQS: Record<string, { q: string; a: string }[]> = {
  ...
  'proteoglycan-salmon-vs-hyaluronic-acid-oral': [
    {
      q: 'プロテオグリカンと経口ヒアルロン酸の違いは？',
      a: '構造とカテゴリが異なります。プロテオグリカンは「コアタンパク+多数の糖鎖（グリコサミノグリカン）」が結合した複合体で、ヒアルロン酸（多糖類単体）とは別カテゴリの成分です。\n\nヒアルロン酸は多糖類単体で1g当たり最大6Lの水分保持が報告される一方、プロテオグリカンはコラーゲンの線維形成支援・抗炎症作用が細胞実験で報告されています。役割分担として、ヒアルロン酸が真皮の水分保持の主柱、プロテオグリカンがコラーゲン産生支援+水分保持の補完という整理が現実的です。\n\n両者とも経口摂取で皮膚改善が報告される小規模RCTがあり、構造的に補完関係のため併用も合理的です。',
    },
    {
      q: '両方併用すると月コストはどのくらいになりますか？',
      a: 'プロテオグリカン（5mg/日・国内製品中心）とヒアルロン酸（120mg/日・iHerbや国内）の併用月コストは¥4,000〜8,000程度が目安です。\n\nプロテオグリカンは抽出収率が低く稀少素材のため、5mg/日でも月¥2,500〜5,000と高めです。ヒアルロン酸経口は¥1,500〜3,000と比較的安価で、コラーゲンペプチド5g/日（¥1,500〜3,000）と組み合わせれば計¥5,000〜8,000の美容スタックになります。\n\n優先順位で迷う場合は、まずコラーゲンペプチド+ヒアルロン酸経口の標準スタックから始め、効果実感後にプロテオグリカンを追加する段階的導入がコスパ的に現実的です。',
    },
    {
      q: 'どちらから始めるべきですか？',
      a: 'エビデンス本数・コスト・入手性で評価すると、まず経口ヒアルロン酸から始めるのが現実的です。\n\n経口ヒアルロン酸はOe 2017・Kawada 2014 などの中規模RCT複数本で皮膚乾燥・小じわ改善が確認され、iHerb・国内ともに製品流通が豊富で月¥1,500〜3,000とコスパが良好です。プロテオグリカンはKakizaki 2017 RCT n=21 が主要根拠で小規模・サンプル数限定で、価格は2〜3倍です。\n\n8〜12週評価して効果実感が乏しい場合に、プロテオグリカン追加か、コラーゲンペプチドへの切り替えを検討する段階的判断が現実的です。',
    },
    {
      q: '効果が出るまで何週間かかりますか？',
      a: '両者とも肌の水分量・キメは4〜8週、皮膚弾力・小じわは8〜12週評価が判定の目安です。\n\nKakizaki 2017（PG 10mg×8週）とOe 2017（HA 120mg×12週）の主要RCTいずれも、12週評価で計測指標の改善が確認されました。短期評価では計測指標の変化を捉えにくいため、最低3ヶ月の継続が現実的です。\n\n3ヶ月継続して肌の手触り・乾燥感に変化が乏しい場合は、用量増量か、コラーゲンペプチド+セラミドへの切り替えを検討します。',
    },
    {
      q: '魚アレルギーがあります。両方避けるべきですか？',
      a: 'プロテオグリカン（鮭由来）は避けてください。経口ヒアルロン酸は由来により判断が分かれます。\n\nプロテオグリカンは鮭の鼻軟骨が主原料で、原料中の微量タンパク質残存により交差反応の可能性があります。鮭アレルギー既往者は避けるべきです。\n\n経口ヒアルロン酸は鶏冠由来・微生物発酵由来があり、微生物発酵由来（多くのiHerb製品）であれば魚アレルギーと無関係です。製品ラベルで「鶏冠由来」「鶏トサカ由来」表記がないものを選べば安全です。',
    },
  ],
  'elastin-peptide-oral-vs-collagen-peptide': [
    {
      q: 'エラスチンとコラーゲンの違いは？',
      a: 'コラーゲンが真皮で「線維束のハリ・引っ張り強度」を担い、エラスチンが「ゴムのような弾力・元に戻る性質」を担う補完関係です。\n\n真皮では両者が網目構造を形成し、コラーゲンが約70-80%、エラスチンが約2-4%と量的にはコラーゲンが圧倒的多数です。コラーゲンが減ると肌のハリが落ち、エラスチンが減ると肌の弾力性・形状回復力が落ちます。両者は加齢で減少しますが、エラスチンは特に修復が困難な特性があります。\n\nペプチド経口摂取は両者ともに皮膚改善が小規模〜中規模RCTで報告されていますが、コラーゲンペプチドの方が研究本数が圧倒的に多く、エラスチンは小規模RCTが中心です。',
    },
    {
      q: 'どちらを優先すべきですか？',
      a: 'エビデンス本数・コスト・効果範囲を総合するとコラーゲンペプチドが優先です。\n\nコラーゲンペプチドは Proksch 2014 RCT n=69・Asserin 2015 RCT n=106・Kim 2018 RCT n=72 など中規模RCTが複数あり、メタ解析（de Miranda 2021）でも保湿・弾力改善のエビデンスが確立しています。月コストも¥1,500〜3,000と安価です。\n\nエラスチンペプチドはShiratsuchi 2016 RCT n=24 と小規模で、月コスト¥3,000〜6,000と高めです。コラーゲン単独で8〜12週評価し、効果実感を見てからエラスチンを追加検討する段階的アプローチが現実的です。',
    },
    {
      q: '両方併用すると効果は重なりますか？',
      a: '両者は構造的に補完関係のため、併用は合理的で効果の重複ではなく強化が期待できます。\n\nコラーゲンが線維束のハリを担い、エラスチンが弾力性を担う役割分担で、機能的に独立した経路です。一部の研究では両者併用群がコラーゲン単独より弾力改善が大きい傾向が報告されていますが、ヒトでの直接比較大規模RCTはまだ限定的です。\n\n併用月コストはコラーゲン5g+エラスチン75mgで¥4,500〜9,000程度です。コスト対効果を重視するなら、まずコラーゲン単独で8〜12週評価が現実的な順序です。',
    },
    {
      q: '効果が出るまで何週間かかりますか？',
      a: '両者とも肌の保湿・キメは4〜8週、皮膚弾力・小じわは8〜12週評価が判定の目安です。\n\nProksch 2014（コラーゲン 2.5g×8週）・Shiratsuchi 2016（エラスチン 75mg×12週）の主要RCTいずれも、8〜12週評価で計測指標の改善が確認されました。短期評価では計測指標の変化を捉えにくく、最低3ヶ月の継続が前提です。\n\n3ヶ月継続して肌の手触り・乾燥感に変化が乏しい場合は、用量増量・ヒアルロン酸経口の追加・セラミド経口への切り替えを検討します。',
    },
    {
      q: 'コラーゲン由来による違いはありますか？',
      a: 'コラーゲンペプチドは魚由来・ブタ由来・ウシ由来があり、それぞれ吸収率・アミノ酸組成・宗教的配慮で違いがあります。\n\n魚由来（マリンコラーゲン）は分子量が小さく吸収率が高い傾向、アレルギー注意。ブタ由来は伝統的でコスト面で優れる、宗教的制約あり。ウシ由来はコラーゲン1型主体で皮膚研究で多用、BSE懸念は現在は管理下で安全。\n\nエラスチンペプチドはカツオ由来が主流で、宗教的配慮が必要な場合は植物由来コラーゲン代替（具体的代替は限定的）を検討します。研究準拠ならカツオ由来エラスチン+任意のコラーゲン由来の組み合わせが現実的です。',
    },
  ],
  'shilajit-fulvic-vs-maca-black-red-yellow': [
    {
      q: 'シラジットとマカの違いは？',
      a: '両者は男性活力サポートで使われますが、由来と作用機序が全く異なる別カテゴリです。\n\nシラジットはヒマラヤ岩盤の樹脂状物質で、フミン酸・フルボ酸・ジベンゾアルファピロン（DBP）が活性指標です。Pandit 2016 RCT n=75 でシラジット500mg×90日で総テストステロン上昇が報告されました。\n\nマカはペルー高地の根菜（Lepidium meyenii）で、多糖類・グルコシノレート・マカミドが活性指標です。Gonzales 2002 RCT n=8 で性欲改善が報告されましたがホルモン値変化は確認されず、作用機序はホルモン経路ではなくアダプトゲン的中枢神経作用と推定されます。\n\n両者は補完的で併用も合理的です。',
    },
    {
      q: 'テストステロン上昇効果はどちらが強いですか？',
      a: '研究データではシラジットの方がテストステロン上昇エビデンスがあります。\n\nPandit 2016 RCT n=75 で精製シラジット500mg/日×90日で、総テストステロン・遊離テストステロン・DHEA上昇が確認されました。一方マカはGonzales 2003 RCT n=20 でテストステロン・LH・FSH等のホルモン値に変化なし・性欲のみ改善というデータです。\n\nテストステロン上昇を主目的とするならシラジット、性欲・スタミナ全般を目的とするならマカが現実的な選び分けです。両者は作用経路が異なるため併用も合理的で、月コストは合計¥3,000〜6,000程度です。',
    },
    {
      q: '副作用と禁忌の違いは？',
      a: '両者とも一般的に忍容性は良好ですが、注意点が異なります。\n\nシラジットは精製品では重金属混入リスクが低いですが、未精製・低品質品では鉛・水銀・ヒ素汚染の報告例があります。PrimaVie®等の規格化抽出品を選ぶことが重要です。鉄サプリ併用注意（フルボ酸の鉄キレート作用）です。\n\nマカは甲状腺機能障害既往者は注意が必要（グルコシノレート含有・甲状腺ホルモン代謝への理論的影響）です。子宮筋腫・乳がん既往者はホルモン感受性で医師相談が現実的です。両者とも妊娠中・授乳中の安全性データは限定的で医師相談が前提です。',
    },
    {
      q: '効果が出るまで何週間かかりますか？',
      a: '両者ともホルモン・性機能改善は8〜12週評価が判定の目安です。\n\nPandit 2016（シラジット）は90日評価で、Gonzales 2002（マカ）も12週評価が主要RCTのタイミングです。短期1〜2週では計測指標の変化を捉えにくく、最低3ヶ月の継続が前提です。\n\n3ヶ月継続して効果実感が乏しい場合は、用量増量（シラジット 500→750mg、マカ 1500→3000mg）・両者併用・トンカットアリ・フェヌグリーク等他選択肢への切り替えを検討します。',
    },
    {
      q: 'どちらから始めるべきですか？',
      a: '目的別の選び方が現実的です。\n\nテストステロン低下・血液検査で低値の方は、シラジットから始めて90日評価が研究データに沿った選択です。\n\n性欲・スタミナ全般の補強で血液検査異常なしの方は、マカから始めて12週評価が現実的です。\n\n両者は作用経路が異なるため併用も合理的で、3ヶ月のトライアル期間でデータを取ってから継続判断するのが良い選び方です。重要なホルモン関連の補強なら、開始前に医師相談・血液検査（テストステロン・LH・FSH・TSH）が望ましいです。',
    },
  ],
  'pha-gluconolactone-vs-glycolic-acid': [
    {
      q: 'PHAとグリコール酸はどう違いますか？',
      a: '両者ともヒドロキシ酸系の角質剥離成分ですが、分子量と作用速度が大きく異なる誤対比の関係です。\n\nPHA（グルコノラクトン）は分子量約180Daと大きく、皮膚への浸透が緩やかで刺激性が低い特性があります。グリコール酸（AHA）は分子量76Daと小さく、速く深く浸透して効果も刺激性も強いです。\n\nGreen 2009 直接比較RCT n=40 で12週評価では、肌キメ改善は両者ほぼ同等で、皮膚刺激スコアはPHAが有意低と報告されました。両者とも継続使用で抗老化・色素沈着改善のエビデンスがあります。',
    },
    {
      q: 'どちらを選ぶべきですか？',
      a: '肌タイプと目的で使い分けます。\n\n敏感肌・酒さ・グリコール酸の刺激で離脱した方はPHAから始めるのが現実的です。Berardesca 2016 で軽度酒さでもPHAの忍容性が確認されています。\n\n健常肌・刺激耐性があり速い効果を求める方はグリコール酸（5〜10%）が選択肢です。研究本数とメタ解析がPHAより豊富で、肌キメ・小じわ・色素沈着の改善エビデンスが確立しています。\n\n両者を併用する場合は、夜PHA毎日+週1〜2回グリコール酸ピーリングという段階的使用も選択肢です。',
    },
    {
      q: '併用は可能ですか？',
      a: '同時併用は皮膚刺激の重複リスクがあるため、時間分離か頻度調整が現実的です。\n\n両者とも角質剥離成分のため、同時塗布で角質バリア破壊が起こる可能性があります。併用するなら、夜PHA毎日+週末グリコール酸ピーリングの頻度分離か、朝PHA+夜グリコール酸の時間分離が選択肢です。\n\n初期は単独使用で皮膚反応を見て、慣れてから併用を検討する段階的アプローチが安全です。皮膚刺激（赤み・剥離・チクチク）が出たら頻度を減らすか中止します。',
    },
    {
      q: '効果が出るまで何週間かかりますか？',
      a: '両者とも肌キメ・色素沈着の改善は8〜12週評価が判定の目安です。\n\nGreen 2009（PHA 8% vs グリコール酸8%・12週）の直接比較RCTでも12週評価が判定タイミングで、両者同等のキメ改善が確認されました。短期4週評価では計測指標の変化を捉えにくいです。\n\n小じわ・色素沈着の改善は12〜24週評価が現実的で、長期継続が前提です。12週評価で効果実感が乏しい場合は、用量増量・他成分（レチノール・ナイアシンアミド）への切り替えを検討します。',
    },
    {
      q: '妊娠中・授乳中はどちらが安全ですか？',
      a: '両者とも安全性データが限定的のため、妊娠中・授乳中の使用は医師相談が前提です。\n\nグリコール酸はFDA pregnancy category B・C相当で、全身吸収量は限定的ですが大規模ヒト試験がありません。PHAも同様に妊娠中ヒト試験データが乏しいです。\n\n妊娠中の肌ケアは、まず保湿剤での皮膚バリア補強を優先し、ヒドロキシ酸類は医師相談で慎重判断するのが現実的です。授乳中も同様で、外用素材の母乳移行は限定的と推定されますが医師判断を推奨します。',
    },
  ],
  'adapalene-01pct-topical-vs-tretinoin-topical': [
    {
      q: 'アダパレンとトレチノインの違いは？',
      a: '両者ともレチノイドのニキビ治療薬ですが、世代と受容体選択性が異なります。\n\nアダパレン（第三世代レチノイド）はRAR-β/γ選択性で、トレチノイン（第一世代レチノイド）のRAR全般作用と比べて皮膚刺激が低いのが特徴です。Otberg 2014 Cochrane（31 RCT統合）で、ざ瘡病変減少効果は両者同等・皮膚刺激スコアはアダパレンが有意低と評価されました。Tu 2010 直接比較RCT n=105 でも同様の結果です。\n\n両者とも妊娠中絶対禁忌（レチノイド催奇形性）・日本では医薬品扱いで皮膚科処方が前提です。',
    },
    {
      q: '抗老化（しわ改善）を求める場合はどちらですか？',
      a: '抗老化（しわ・光老化）目的ならトレチノインの方がエビデンス本数が豊富です。\n\nトレチノインは抗老化（しわ改善・コラーゲン産生）の長期使用RCTが30年以上の蓄積があり、Kligman 1986・Weiss 1988 など主要研究で確立されています。Olay/化粧品RetinolもトレチノインのOTC変換型として位置づけられます。\n\nアダパレンの抗老化エビデンスは限定的で、ざ瘡治療が主目的です。抗老化目的でアダパレンを使う研究もありますが、トレチノインほどのデータ蓄積はありません。\n\nニキビ+抗老化を狙うならトレチノイン、ニキビ単独で刺激性を抑えたいならアダパレンが現実的な選び分けです。',
    },
    {
      q: '初心者はどちらから始めるべきですか？',
      a: '初心者・敏感肌の方はアダパレン0.1%から始めるのが現実的です。\n\nアダパレンの方がA1反応（初期皮膚刺激）が穏やかで、トレチノインで離脱経験のある方の代替選択肢として確立しています。皮膚科医も初心者にはアダパレンか低濃度トレチノイン（0.025%）を処方することが多いです。\n\nアダパレン0.1%で4〜8週使用して皮膚刺激に慣れ、効果実感が乏しい場合や抗老化目的が強い場合に、皮膚科医と相談してトレチノインへの切り替えを検討します。',
    },
    {
      q: '妊娠中・授乳中は両方とも使えませんか？',
      a: '両者とも妊娠中・妊娠予定者は絶対禁忌、授乳中も避けてください。\n\nレチノイド類は催奇形性（先天奇形リスク）が報告されており、外用でも全身吸収による胎児リスクが懸念されます。FDA pregnancy category C（外用レチノイド）・D（内服イソトレチノイン）です。\n\n妊娠予定があれば、開始前に皮膚科医に相談し避妊期間と使用期間を計画してください。妊娠が判明したら即座に使用中止します。\n\n妊娠中・授乳中のニキビケアは、サリチル酸・アゼライン酸・グリコール酸など妊娠中も使用可能とされる成分を皮膚科医と相談して選択します。',
    },
    {
      q: '日本では市販で買えますか？',
      a: '両者とも日本では医薬品扱いで、皮膚科医の処方箋なしには購入できません。\n\nアダパレン0.1%（処方名「ディフェリンゲル」）・アダパレン+BPO配合剤（処方名「エピデュオゲル」）は2008年に承認されました。\n\nトレチノインは2024年現在、ざ瘡治療用としての日本承認はなく、皮膚科の自由診療または個人輸入が選択肢です。海外（アメリカ・ヨーロッパ）ではOTCで購入可能な国もあります。\n\n日本居住者は皮膚科を受診して保険適用での処方を受けるのが安全で経済的に現実的です。本ページは情報提供のみで、未承認薬の個人輸入推奨ではありません。',
    },
  ],
  'benzoyl-peroxide-25pct-vs-salicylic-acid': [
    {
      q: '過酸化ベンゾイルとサリチル酸はどう違いますか？',
      a: '両者とも外用ニキビ治療薬として広く使われますが、作用機序と対象ニキビが異なる別カテゴリです。\n\n過酸化ベンゾイル（BPO）は活性酸素発生による C. acnes 殺菌が主作用で、炎症性ニキビ（赤ニキビ・膿疱）に有効です。Yang 2020 Cochrane n=12,352 で第一選択評価が確立されています。\n\nサリチル酸（BHA）は油溶性で毛穴内部の角質と皮脂を溶かす角質剥離が主作用で、非炎症性ニキビ（コメド・面ぽう）・毛穴詰まりに有効です。\n\n両者は補完的で、炎症性ニキビにBPO・コメド予防にサリチル酸という使い分けが現実的です。',
    },
    {
      q: '併用は可能ですか？',
      a: '同時併用は皮膚刺激の重複リスクがあるため、時間分離が現実的です。\n\n両者とも刺激性があり、同時塗布で皮膚バリア破壊や乾燥が強くなる可能性があります。併用するなら、朝サリチル酸+夜BPOの時間分離か、皮膚刺激が強い場合は隔日使用で頻度を分けます。\n\n皮膚科医処方の場合は、BPO+アダパレン配合剤を主軸にサリチル酸を朝にスポット使用といった段階的アプローチが選択肢です。皮膚科医と相談して個別判断するのが安全です。',
    },
    {
      q: '日本で買いやすいのはどちらですか？',
    ...
    {
      q: '妊娠中はどちらが安全ですか？',
      a: '妊娠中はサリチル酸の方が安全性データが豊富で、低濃度ならば使用可能とされます。\n\nサリチル酸2%以下の外用は、米国産婦人科学会（ACOG）でも妊娠中の使用が一般的に許容されています。広範囲・高濃度（10%超）は経皮吸収リスクがあるため避けます。\n\nBPOは妊娠中の安全性データが限定的で、外用は理論的に安全と推定されますが大規模ヒト試験がありません。皮膚科医・産婦人科医と相談して判断するのが現実的です。\n\n妊娠中ニキビケアは、低濃度サリチル酸・アゼライン酸を皮膚科医相談で選択するのが安全側の判断です。',
    },
    {
      q: '効果が出るまで何週間かかりますか？',
      a: '両者とも臨床的な病変減少は8〜12週評価が判定の目安です。\n\nBPOはYang 2020 Cochrane で8〜12週評価でプラセボより有意な病変減少が確認されました。細菌数減少は2週で1.5 log10低下（Eady 1994）と早期から確認できますが、見た目の改善は8週以降が主体です。\n\nサリチル酸はZander 1992 や複数の小規模試験で4〜12週評価が一般的で、コメド改善は4〜8週で実感する方が多いです。\n\n12週評価で改善が乏しい場合は、皮膚科医と相談して用量増量・他治療併用・別治療への切り替えを検討します。',
    },
  ],
  'benzoyl-peroxide-25pct-vs-adapalene-01pct-topical': [
    {
      q: 'BPOとアダパレンは併用すべきですか？',
      a: '両者は皮膚科ガイドラインで併用が推奨される補完関係です。配合剤（アダパレン+BPO配合剤・処方名「エピデュオゲル」）も日本で処方薬として承認されています。\n\nThiboutot 2006 RCT n=653 で配合剤が単剤より病変改善が優れ、Pariser 2014 で6ヶ月維持療法の再発率低下も確認されています。BPOがC. acnes殺菌・抗炎症を担い、アダパレンが角化正常化・コメド予防を担う機序的補完です。\n\n配合剤を1日1回夜が標準で、皮膚科医処方で保険適用です。単剤併用より配合剤が用量管理と継続性で優れます。',
    },
    {
      q: '配合剤と単剤併用ではどちらが良いですか？',
      a: '皮膚科治療では配合剤（エピデュオゲル）が第一選択です。\n\n配合剤の利点は、(1) 用量比が研究で最適化されている（アダパレン0.1%+BPO 2.5%）、(2) 1回塗布で完結し継続性が高い、(3) 保険適用で経済的、(4) 皮膚刺激が分散される、です。\n\n単剤併用（朝にBPO・夜にアダパレン等）は、皮膚刺激が強い場合の頻度調整や、配合剤入手困難時の選択肢です。皮膚科医の指示に従って個別判断するのが現実的で、自己判断で時間分離するより配合剤処方を受けるのが安全です。',
    },
    {
      q: '皮膚刺激（A1反応）が強い場合はどうすればよいですか？',
      a: '配合剤・単剤併用ともに初期2〜4週は皮膚刺激（赤み・乾燥・剥離・チクチク）が起きやすく、ほぼ全例で4週以降に軽減します。\n\n対処法として、塗布前後の保湿剤（セラミド・ヒアルロン酸含有）使用で皮膚バリアを補強します。隔日使用・短時間接触法（10〜30分後に洗い流す）への変更も選択肢で、皮膚科医と相談して頻度調整します。\n\n日中は必ずSPF30以上の日焼け止めを使用してください（光感受性亢進）。4週以上経っても刺激が強く改善しない場合は、皮膚科医と相談して別治療への切り替えを検討します。',
    },
    {
      q: '効果が出るまで何週間かかりますか？',
      a: '配合剤の効果実感は、目標により評価期間が異なります。\n\n非炎症性病変（コメド・面ぽう）は4〜8週で減少を感じやすく、炎症性病変（赤ニキビ・膿疱）は8〜12週評価が主要RCTの判定タイミングです。Thiboutot 2006 では12週評価で50％以上の病変減少が65％の患者で達成されました。\n\n初期2〜4週は新しい病変が一時的に増えることがあり（プッシュアウト現象）、これは古いコメドが表面化する現象で悪化ではなく治療効果の一過程と考えられています。\n\n12週評価で改善が乏しい場合は、皮膚科医と相談して用量増量・他治療併用を検討します。',
    },
    {
      q: '妊娠中・授乳中は使えますか？',
      a: '配合剤を含めて妊娠中・妊娠予定者は絶対禁忌（アダパレンのレチノイド催奇形性）、授乳中も避けてください。\n\nレチノイド類は催奇形性が報告されており、外用でも全身吸収による胎児リスクが懸念されます。BPO単独は理論的に外用安全と推定されますが、配合剤を含むレチノイド成分使用は妊娠中NGです。\n\n妊娠予定があれば、開始前に皮膚科医に相談し避妊期間と使用期間を計画してください。妊娠中・授乳中のニキビケアは、サリチル酸・アゼライン酸・グリコール酸など妊娠中も使用可能とされる成分を皮膚科医と相談して選択します。',
    },
  ],
  'd-mannose-vs-cranberry-extract': [
    {
      q: 'D-マンノースとクランベリーはどう違いますか？',
      a: '両者ともUTI（尿路感染症）予防成分ですが、作用機序と効果時間軸が異なります。\n\nD-マンノース（2g/日）は単糖類で、尿中に排泄されて大腸菌のFimH線毛に直接結合し、尿路上皮細胞への付着を物理的にブロックする即効性タイプです。\n\nクランベリーエキス（PAC換算36mg/日）は植物ポリフェノールで、A型プロアントシアニジン（PAC）が広域な菌種への作用と長期継続による予防の時間軸タイプです。\n\nKranjcec 2014 RCT n=308 で、両者ともに無治療より再発性UTI予防効果が確認されました。',
    },
    {
      q: '効果はどちらが強いですか？',
      a: 'Kranjcec 2014 直接比較RCT n=308 で、D-マンノース2g/日 vs クランベリーは予防効果がほぼ同等と報告されました。\n\nD-マンノースは抗生剤予防投与（ニトロフラントイン）と同等の予防効果が確認され、副作用が少ない点で評価されています。クランベリーはJepson 2012 Cochrane（24 RCT・n=4,473）で再発相対リスク約14％低下が確認されました。\n\n選び方として、D-マンノースは即効性と高い予防効果が優先される方、クランベリーは長期継続のしやすさ・経済性が優先される方に向きます。再発頻度が高い場合は、両者の併用も選択肢に入ります。',
    },
    {
      q: '併用は可能ですか？',
      a: '両者は作用機序が異なるため併用は安全で合理的です。\n\nD-マンノースが大腸菌の物理的ブロック（即効）、クランベリーが広域な菌種への作用と長期予防（蓄積）と補完関係にあります。重篤な相互作用報告はなく、同時摂取が可能です。\n\n併用月コストはD-マンノース2g/日（¥3,000〜6,000）+クランベリーPAC換算36mg/日（¥2,000〜4,000）で計¥5,000〜10,000程度です。再発頻度が月1回以上の難治例で、医師相談の上で併用を検討するのが現実的です。',
    },
    {
      q: '急性膀胱炎の治療には使えますか？',
      a: '両者とも急性治療には向きません。抗生剤治療が第一選択です。\n\nJepson 2012 Cochraneでもクランベリーは「予防的補助」としての効果が確認された一方、急性症状の改善には無効と評価されました。D-マンノースも同様に予防が主目的で、急性治療データは限定的です。\n\n排尿時痛・血尿・発熱が出ている場合は泌尿器科を受診して抗生剤治療を受けるべきです。両者の位置づけは、急性期治療後の再発予防・抗生剤予防投与を避けたい方の補助です。',
    },
    {
      q: '効果が出るまでどのくらいの期間が必要ですか？',
      a: '両者とも予防効果は12〜24週の継続摂取で評価が現実的です。\n\nD-マンノースは予防作用が即効性（数時間〜数日）で、症状再発の頻度低下として6ヶ月評価が主要RCTタイミングです。クランベリーもJepson 2012 で平均6ヶ月以上の継続で効果が確認された試験が多数です。\n\n3ヶ月以内の短期評価では効果判定が難しく、最低6ヶ月の継続摂取が前提です。6ヶ月継続して再発頻度に変化が乏しければ、もう一方への切り替えや医師相談を検討します。',
    },
    {
      q: '妊娠中の使用はどうですか？',
      a: '妊娠中はUTIリスクが高まるため予防の必要性は高いですが、サプリ使用は医師相談の上で判断してください。\n\nD-マンノースは妊娠中の安全性データが限定的ですが、消化管吸収率が低く尿中に直接排泄される特性から理論的に安全側と推定されます。クランベリーも食事範囲は安全とされますが、サプリレベルの安全性データは不足です。\n\n妊娠中UTI予防は、まず水分摂取の確保・排尿習慣の改善・性交渉後の排尿習慣を優先し、サプリは医師判断で選択肢とするのが現実的です。授乳中も同様で医師相談を推奨します。',
    },
  ],
  'riboside-trigonelline-vs-nmn': [
    {
      q: 'トリゴネリンとNMNはどう違いますか？',
      a: '両者ともNAD+前駆体ですが、由来と代謝経路が異なる別カテゴリの成分です。\n\nトリゴネリンは天然のコーヒー・フェヌグリーク由来のアルカロイドで、吸収後にニコチン酸代謝に変換される間接的なNAD+前駆体です。Membrez 2024 マウス研究でトリゴネリンが筋肉NAD+上昇と運動能力改善を確認しました。\n\nNMN（ニコチンアミドモノヌクレオチド）は直接的なNAD+前駆体で、ニコチンアミドリボシド（NR）からの直接合成経路を取ります。Yoshino 2021 RCT n=10 で経口250mg×10週で血中NAD+上昇が確認されました。\n\n研究蓄積はNMNが優位で、トリゴネリンは新興フロンティアです。',
    },
    {
      q: 'どちらを選ぶべきですか？',
      a: '研究本数とエビデンス確立度を考慮するとNMNが現状では優先選択です。\n\nNMNはYoshino 2021・Igarashi 2022・Nadeeshani 2022 など複数のヒトRCTが報告されており、血中NAD+上昇・運動能力・睡眠改善などの効果データが蓄積しています。月コストは¥3,000〜8,000程度です。\n\nトリゴネリンはMembrez 2024 が代表的研究ですがマウス段階中心で、ヒトRCTは限定的です。コーヒー摂取（1日2〜3杯）で自然に5〜30mg/日が摂取されるため、サプリ単独より日常的コーヒー摂取での自然摂取が現実的な選択肢です。\n\n新興フロンティアとしての関心度は高いものの、サプリ投資としてはNMNが優先される段階です。',
    },
    {
      q: '併用は可能ですか？',
      a: '両者は経路が異なるため併用は理論的に合理的ですが、ヒトでの併用研究はまだ限定的です。\n\nトリゴネリンが間接経路、NMNが直接経路から NAD+ 合成を補強する補完関係にあります。重篤な相互作用報告はなく、同時摂取で大きなリスクは想定されません。\n\nコーヒー摂取（自然なトリゴネリン摂取）+ NMNサプリの組み合わせは現実的なスタックです。サプリ併用月コストは¥5,000〜12,000程度になります。\n\nどちらかを優先するならNMNを主軸とし、トリゴネリンは日常的コーヒー摂取で自然補給するのが経済的に現実的です。',
    },
    {
      q: '効果が出るまで何週間かかりますか？',
      a: 'NMNの効果実感は、目標により評価期間が異なります。\n\n血中NAD+上昇は数日〜数週で計測可能で、Yoshino 2021では10週評価が判定タイミングでした。エネルギー実感・運動能力改善は4〜12週で報告される傾向があります。\n\nトリゴネリンはヒトRCTが限定的で、評価期間の確立データが不足です。コーヒー由来の自然摂取は継続的な摂取が前提で、変化を実感するのは難しいかもしれません。\n\n12週評価で効果実感が乏しい場合は、用量増量・他NAD+前駆体（NR・ナイアシン）への切り替え・併用を検討します。',
    },
    {
      q: '副作用と長期安全性の違いは？',
      a: '両者とも一般的に忍容性は良好です。\n\nNMNはYoshino 2021・Igarashi 2022 で250-500mg/日まで重篤な副作用報告はなく、軽度GI不快感が報告される程度です。長期摂取（1年以上）の安全性データは中程度で蓄積中です。\n\nトリゴネリンは日常的コーヒー摂取での蓄積データが豊富で、安全性プロファイルは良好です。サプリレベルの長期データは限定的です。\n\n両者とも妊娠中・授乳中の安全性データは不足で医師相談前提です。糖尿病治療中はNMNが血糖代謝への影響理論的可能性で医師相談が現実的です。',
    },
  ],
  'dasatinib-senolytic-vs-fisetin': [
    {
      q: 'ダサチニブとフィセチンはどう違いますか？',
      a: '両者ともセノリティクス（老化細胞除去）作用が研究されていますが、法的位置と入手難度が大きく異なります。\n\nダサチニブは処方抗がん剤で、senolytic 用途では D+Q（ダサチニブ100mg+ケルセチン1000mg）プロトコルとして月1-2回断続使用が研究されています。Justice 2019 オープン試験 n=14 でD+Qが老化細胞マーカー低下を確認しました。日本では医師管理下でのみ使用可能です。\n\nフィセチンはイチゴ・りんご由来のフラボノイドで、健康食品として自己選択で購入可能です。Yousefzadeh 2018 マウス研究で経口で寿命延長・健康スパン改善が報告されました。\n\n法的位置が完全に異なる別カテゴリです。',
    },
    {
      q: 'ヒトでの効果エビデンスはどちらが強いですか？',
      a: 'ヒト試験データはダサチニブ（D+Qプロトコル）の方が現状では蓄積があります。\n\nダサチニブはJustice 2019（n=14・特発性肺線維症患者）・Hickson 2019（n=20・糖尿病性腎症患者）でD+Qプロトコルの老化細胞マーカー低下が報告されています。ただし対象は処方薬適応の重篤疾患患者で、健常者へのsenolytic用途は研究段階です。\n\nフィセチンはヒトRCTが限定的で、Yousefzadeh 2018 マウス研究が中心です。100mg/日経口での健康効果は理論的・推定段階で、確立したヒトエビデンスはまだ不足です。\n\n両者とも研究フロンティアで、健康人での確実な効果データは未確立です。',
    },
    {
      q: '一般人が試せるのはどちらですか？',
      a: '入手難度を考えると、フィセチンが一般人の現実的な選択肢です。\n\nフィセチン100mg/日のサプリはiHerb等で月¥2,000〜5,000で購入可能で、健康食品として自己選択できます。重篤な副作用報告はなく、忍容性は良好です。\n\nダサチニブは処方抗がん剤で、senolytic 用途での処方は日本では一般的でなく、研究プロトコル参加か自由診療での個別判断となります。月コストは数万円以上で、医師管理下でのみ使用可能です。\n\nセノリティクス効果は両者ともヒトでの確立データが限定的で、明確な期待効果よりも研究的関心が中心の段階です。',
    },
    {
      q: '副作用と禁忌の違いは？',
      a: '両者の安全性プロファイルは大きく異なります。\n\nダサチニブは抗がん剤として骨髄抑制・胸水・心毒性等の重篤副作用が報告されており、医師管理下での慎重使用が前提です。妊娠中絶対禁忌・授乳中禁忌です。\n\nフィセチンは健康食品として一般的に忍容性が良好で、軽度GI不快感の報告がある程度です。ただし長期高用量（500mg/日以上）の安全性データは限定的です。抗凝固薬併用時は理論的注意（フラボノイドの軽度な抗凝固作用）が必要です。\n\n両者とも妊娠中・授乳中・小児の安全性データは不足です。',
    },
    {
      q: 'どちらを試すべきですか？',
      a: '健康人での自己選択範囲では、現状フィセチンが現実的な選択肢ですが、明確な期待効果は研究段階です。\n\n本格的なsenolyticプロトコル（D+Q）への関心がある方は、まず信頼できる医師（抗加齢医学会認定医・自由診療クリニック等）に相談するのが現実的です。研究プロトコル参加も選択肢です。\n\nフィセチンは健康食品として月¥2,000〜5,000で試せますが、効果が確実に得られる段階ではないことを理解した上での投資判断が現実的です。\n\nどちらも研究フロンティア段階で、確立した臨床推奨はまだありません。本ページは情報提供のみで医療判断は処方医にお任せします。',
    },
  ],
  'propolis-vs-manuka-honey-mg-rich': [
    {
      q: 'プロポリスとマヌカハニーはどう違いますか？',
      a: '両者とも蜂由来の天然防御物質ですが、活性成分と作用範囲が異なる別カテゴリです。\n\nプロポリスはミツバチが植物樹脂と蜂蝋から作る巣材で、フラボノイド（ガランギン・ピノセンブリン・CAPE）が活性指標です。歯肉炎・化学療法口内炎で複数RCT、全身免疫サポートに研究蓄積があります。\n\nマヌカハニー（高MGO）はニュージーランド産マヌカの花蜜から作る蜂蜜で、メチルグリオキサール（MGO）が活性指標です。喉ケア・創傷ケア・上気道感染症補助で研究蓄積があります。\n\n両者とも蜂アレルギー絶対禁忌・1歳未満禁忌で安全性プロファイルは共通します。',
    },
    {
      q: 'どちらを選ぶべきですか？',
      a: '目的別の使い分けが現実的です。\n\n歯肉炎・歯周病・口内炎ケアにはプロポリスが選択肢です。Anauate-Netto 2014 RCTでプロポリスマウスウォッシュ7日で歯肉炎改善、Hwu 2014 Cochrane で口内炎軽減が確認されました。月コスト¥2,000〜5,000程度です。\n\n喉ケア・上気道感染症・創傷ケアにはマヌカハニーが選択肢です。Abuelgasim 2020 メタ解析で咳症状改善、Khanal 2015 RCT で化学療法口内炎軽減が確認されました。月コスト¥3,000〜8,000（MGO 250+〜400+）程度です。\n\n両者は補完的で併用も合理的です。',
    },
    {
      q: '併用は可能ですか？',
      a: '両者は作用範囲が補完的で併用は合理的です。\n\nプロポリスが全身的な免疫サポート・抗炎症・口腔ケアを担い、マヌカハニーが喉ケア・上気道感染症補助・創傷ケアを担う棲み分けです。重篤な相互作用報告はなく、同時摂取で大きなリスクは想定されません。\n\nただし両者とも蜂アレルギー絶対禁忌で、片方でアレルギー反応が出た場合は両方とも中止します。冬季・繁忙期の集中的併用や、創傷+口腔症状の同時ケアといった用途で併用が選択肢になります。',
    },
    {
      q: '蜂アレルギーがあります。両方避けるべきですか？',
      a: '両者とも蜂アレルギー既往者は絶対に避けてください。\n\nプロポリスもマヌカハニーも蜂由来のタンパク質・酵素を含み、蜂毒・ローヤルゼリー・他の蜂産物との交差反応性でアナフィラキシー報告例があります。重篤化リスクがあります。\n\n花粉症（ポプラ・松・ブタクサ等）既往者・喘息既往者も注意が必要です。症状発現時は口腔・唇の腫脹・喉違和感・呼吸困難・蕁麻疹等が出現したら即座に中止・救急受診します。エピペン保持者は速やかに使用してください。\n\n代替候補としては、口腔ケアならクロルヘキシジン・キシリトール、喉ケアなら亜鉛トローチ・カモミールティーが現実的です。',
    },
    {
      q: '糖尿病があります。マヌカハニーは避けるべきですか？',
      a: 'マヌカハニーは天然糖質（フルクトース約40％・グルコース約30％）を多く含み、血糖値を上昇させます。糖尿病・血糖管理中の方は医師相談が前提です。\n\nGI値はマヌカハニーで約50前後と報告されますが、血糖反応は個人差が大きく、糖尿病の方は連用前に血糖モニタリングが必要です。1日5-10g程度は健常者でも問題ない範囲ですが、糖尿病の方は1日1-2gの少量を医師判断で検討します。\n\n糖尿病の方の喉ケア・免疫サポート目的なら、糖質非含有のプロポリス（カプセル・スプレー）の方が血糖管理に適している場合があります。亜鉛トローチも糖質含有量が少なく代替候補です。',
    },
  ],
  'schisandra-vs-milk-thistle': [
    {
      q: '五味子とミルクシスルはどう違いますか？',
      a: '両者とも肝サポートハーブですが、作用範囲とエビデンスの深さが異なる別カテゴリです。\n\n五味子（Schisandra chinensis）は伝統中医のアダプトゲンハーブで、肝代謝・耐久力・ストレス耐性・認知機能全般に幅広く作用するとされます。Panossian 2008 で適応原作用が報告されました。\n\nミルクシスル（Silybum marianum）はシリマリン（フラボノリグナン）を主要活性成分とし、C型肝炎・肝硬変・薬剤性肝障害に肝特異的エビデンスが蓄積しています。Saller 2007 メタ解析でアルコール性肝障害への効果が報告されました。\n\nエビデンス本数は肝特異的にミルクシスルが優位、適応原全般は五味子が選択肢です。',
    },
    {
      q: '慢性肝疾患の補助にはどちらが選ばれますか？',
      a: '慢性肝疾患（C型肝炎・肝硬変・脂肪肝等）の補助にはミルクシスルが研究蓄積で優位です。\n\nPares 1998 RCT n=200 でアルコール性肝硬変患者にシリマリン420mg/日×2年で生存率改善傾向が報告されました。Loguercio 2008 で非アルコール性脂肪肝（NAFLD）への効果も確認されています。\n\nただし慢性肝疾患は医師管理下での治療が前提で、サプリ単独での自己治療は危険です。シリマリンは標準治療の補助として、肝内科医に相談の上で導入することが現実的です。\n\n五味子は肝疾患よりもストレス・疲労・認知サポートが主目的で、肝サポートの代替選択肢ではありません。',
    },
    {
      q: '両方併用は可能ですか？',
      a: '両者は作用範囲が異なるため併用は理論的に合理的ですが、ヒトでの併用研究は限定的です。\n\nミルクシスルが肝特異的、五味子がストレス・適応原と棲み分けがあり、重篤な相互作用報告はありません。同時摂取で大きなリスクは想定されません。\n\n慢性肝疾患患者は、まずミルクシスルを医師相談で標準治療補助として導入し、ストレス併発があれば五味子の追加を検討する段階的アプローチが現実的です。\n\n肝疾患治療薬（インターフェロン・直接作用型抗ウイルス薬DAA等）との併用は CYP3A4 阻害の理論的可能性で医師相談が必須です。',
    },
    {
      q: '副作用と禁忌の違いは？',
      a: '両者とも一般的に忍容性は良好ですが、注意点があります。\n\nミルクシスルはキク科植物アレルギー既往者は注意（ブタクサ・カモミール等との交差反応性）です。胆石症既往者は胆汁分泌促進作用で症状悪化の可能性があり医師相談が前提です。妊娠中・授乳中の安全性データは限定的です。\n\n五味子は妊娠中禁忌（子宮収縮作用の理論的可能性）です。GERDの方は胃酸分泌刺激で症状悪化の可能性があります。CYP3A4阻害でカルバマゼピン・タクロリムス・シクロスポリン等の血中濃度上昇報告例があり医師相談が必須です。\n\n両者とも医薬品との相互作用注意が共通します。',
    },
    {
      q: '効果が出るまで何週間かかりますか？',
      a: '両者とも肝機能マーカー（AST・ALT・GGT）の改善は8〜24週評価が判定の目安です。\n\nミルクシスルはSaller 2007 メタ解析や複数RCTで12週評価が主要タイミングで、長期で1年以上の継続研究も多いです。Pares 1998 では2年継続で生存率改善が確認されました。\n\n五味子は適応原作用のため自覚症状（疲労・ストレス耐性）は4〜8週で実感する方が多く、肝機能マーカーの変化は12〜24週評価です。\n\n肝疾患患者は3ヶ月ごとの定期血液検査で経過観察し、改善傾向があれば継続、悪化があれば医師判断で中止・他治療への切り替えを検討します。',
    },
  ],
  'vinpocetine-vs-citicoline': [
    {
      q: 'ビンポセチンとシチコリンはどう違いますか？',
      a: '両者とも認知サポート成分ですが、作用機序が全く異なる誤対比の関係です。\n\nビンポセチンはツルニチニチソウ由来アルカロイドで、PDE1阻害により脳血流改善・cGMP上昇を介して認知機能をサポートする血流タイプです。Szatmari 2003 メタ解析で軽度認知改善が報告されました。\n\nシチコリンは細胞膜リン脂質（PC・PE）の前駆体で、アセチルコリン合成基質を提供する細胞膜タイプです。Spiers 1996 RCT n=84 で記憶改善が確認されました。\n\n両者は補完的で併用も合理的です。',
    },
    {
      q: 'どちらを選ぶべきですか？',
      a: '目的別の使い分けが現実的です。\n\n脳血流が低下している方（高齢・高血圧・脳梗塞既往）はビンポセチンが選択肢で、Szatmari 2003 メタ解析で軽度〜中等度認知障害への効果が報告されています。月コスト¥2,000〜5,000程度です。\n\nアセチルコリン経路サポート・記憶改善が目的の方はシチコリンが選択肢で、Spiers 1996・Alvarez-Sabin 2013 で記憶・注意改善のエビデンスがあります。月コスト¥3,000〜8,000程度です。\n\n両者の併用も補完的で合理的ですが、月コストは¥5,000〜12,000程度になります。',
    },
    {
      q: '併用は可能ですか？',
      a: '両者は作用機序が異なるため併用は安全で合理的です。\n\nビンポセチンが脳血流・cGMP経路、シチコリンが細胞膜・アセチルコリン経路と補完関係にあります。重篤な相互作用報告はなく、同時摂取で大きなリスクは想定されません。\n\n高齢者・軽度認知障害の方の認知サポートスタックとして、ビンポセチン30mg/日+シチコリン500mg/日の併用が選択肢になります。アルファGPC（コリン前駆体）との3者併用も合理的です。\n\nただし両者とも認知症の治療代替ではなく、医師診断・治療を受けている方は処方薬を最優先します。',
    },
    {
      q: '副作用と禁忌の違いは？',
      a: '両者とも一般的に忍容性は良好ですが、注意点が異なります。\n\nビンポセチンは出血傾向増加の理論的可能性があり、抗凝固薬（ワルファリン・DOAC等）・抗血小板薬服用中は医師相談が前提です。FDAは妊娠中の使用について警告を出しています。\n\nシチコリンは軽度GI不快感・頭痛の報告がある程度で、安全性プロファイルは良好です。糖尿病治療中は血糖代謝への理論的影響で医師相談が現実的です。\n\n両者とも妊娠中・授乳中の安全性データは限定的で医師相談前提です。認知症患者は処方薬治療を最優先し、サプリは補助的位置づけです。',
    },
    {
      q: '効果が出るまで何週間かかりますか？',
      a: '両者とも認知機能の改善は8〜24週評価が判定の目安です。\n\nビンポセチンはSzatmari 2003 メタ解析や複数RCTで12週評価が主要タイミングです。脳血流改善の自覚（頭がスッキリ等）は数週で実感する方もいますが、計測指標の変化は8〜12週評価です。\n\nシチコリンはSpiers 1996・Alvarez-Sabin 2013 で12週評価が一般的で、記憶・注意の改善実感は4〜12週で報告される傾向があります。\n\n12〜24週評価で効果実感が乏しい場合は、用量増量・併用・アルファGPC・バコパへの切り替えを検討します。長期摂取の認知症予防効果データは中程度で蓄積中です。',
    },
  ],
  ...
  'algal-oil-vs-omega3': [
    {
      q: '藻類オイルと魚油オメガ3の違いは？',
      a: '原料・水銀汚染リスク・DHA/EPA 比・コスト・ヴィーガン適合の 5 軸で異なります。\n\n藻類オイル（algal oil）は微細藻類（Schizochytrium・Crypthecodinium）培養由来で、魚を経由せず DHA/EPA を直接生産するヴィーガン代替形態。閉鎖系培養タンク内生産のため水銀・PCB・ダイオキシン汚染リスクが構造的にゼロが最大の差別化点。DHA 主体で EPA 含有は少なめ。月コスト¥3,500-5,500。\n\n魚油オメガ3 は青魚（イワシ・サバ・アンチョビ等）由来で大規模 RCT 蓄積が世界最厚（GISSI-Prevenzione 1999 Lancet n=11,324 で全死亡 -20% 等）、EPA/DHA 混合で抗炎症 EPA と脳神経 DHA の両方を含む。水銀・PCB 汚染リスクが構造的に存在（食物連鎖の上位ほど蓄積）し IFOS 認証品で大幅低減できるが完全ゼロではない。月コスト¥1,000-2,500とコスパ良好。\n\n「魚を経由するか否か」が両者を分ける本質的構造の違いです。',
    },
    {
      q: '藻類オイルと魚油は効果が同等？論文ではどう評価されている？',
      a: '血中 DHA 上昇・心血管・脂質改善の主要アウトカムで同等が論文上の評価です。\n\nSanders 2006 Am J Clin Nutr RCT n=79 12 週で藻類由来 DHA 200mg/日を 12 週摂取した群が血中 DHA 濃度を魚油同等用量と統計的に有意差なく上昇させたことが確認されました。Bernstein 2012 J Acad Nutr Diet メタ解析では藻類 DHA の心血管・脂質（中性脂肪 TG 低下）・血圧改善が魚油同等と評価されています。Geppert 2006 Br J Nutr RCT 妊娠中女性 200mg/日 algal DHA で安全性・血中 DHA 上昇・新生児の血中 DHA 濃度上昇も確認され、米国産科婦人科学会のヴィーガン妊婦向け推奨でも algal oil が DHA 源として記載されています。\n\nただし EPA 主体の介入（高純度 EPA REDUCE-IT 2019 NEJM 4g/日で主要心血管イベント -25% 等）は魚油・処方薬の領域で、algal oil は DHA 中心のため EPA 抗炎症作用を最大化したい人は魚油優位です。',
    },
    {
      q: '水銀・PCB 汚染リスクはどれくらい？魚油でも安全？',
      a: '魚油は IFOS 認証品なら実用上は安全ですが、構造的にゼロにはなりません。\n\n魚油の水銀リスク：青魚（イワシ・サバ・アンチョビ等の小型魚）由来の魚油は大型魚（マグロ・カジキ）より食物連鎖が短く水銀蓄積は限定的です。IFOS（International Fish Oil Standards 第三者認証）5 つ星品は水銀・PCB・ダイオキシン・酸化指標が世界基準（米国 CRN・FDA・EPA・WHO 基準）の 10 分の 1 以下に管理されており、Nordic Naturals・Carlson Labs 等の高品質ブランドはほぼ全製品が IFOS 認証取得済。日常的な摂取で健康リスクが顕在化する事例は実質ありません。\n\n藻類オイルの構造的優位：培養タンク内・閉鎖系生産でトレーサビリティが完備されており、水銀・PCB・ダイオキシン汚染リスクが構造的にゼロ。妊娠中・授乳中・高頻度の魚摂取者（週 4 回以上の青魚食習慣がある場合の追加サプリ摂取）・有機水銀感受性が高い人にとっては妥当な選択肢です。\n\n「魚油は IFOS 認証品なら実用上安全・藻類オイルは構造的にゼロ」の差を踏まえた価値判断です。',
    },
    {
      q: '妊娠中・授乳中はどちらを選ぶべき？',
      a: '産科医相談前提で、ヴィーガン・水銀過敏な方は藻類オイル、それ以外は IFOS 認証魚油 or 藻類オイルどちらも妥当です。\n\n妊娠中の DHA 重要性：DHA は胎児の脳・網膜の神経発達に必須で、米国産科婦人科学会・日本産科婦人科学会・WHO 等は妊娠中・授乳中 200-300mg/日の DHA 摂取を推奨しています。Geppert 2006 Br J Nutr RCT で藻類 DHA 200mg/日が血中 DHA 上昇・新生児の血中 DHA 濃度上昇・安全性が確認されています。\n\n藻類オイル優位の理由：①水銀・PCB 汚染リスクが構造的ゼロで妊娠中の有機水銀曝露リスクを完全回避、②ヴィーガン適合で動物性ソース回避、③魚臭いげっぷが少ないため妊娠初期のつわり時期にも続けやすい。月コスト¥3,500-5,500 のプレミアムは妊娠期 10 ヶ月の総額¥35,000-55,000 で実用的な範囲。\n\n魚油の選択：IFOS 認証品（Nordic Naturals Prenatal DHA 等の妊婦向け規格品）なら水銀リスクが大幅低減され、EPA も同時摂取できる利点があります。\n\n大型魚（マグロ・カジキ等）の食事摂取は妊娠中要注意で、サプリは小型魚由来魚油 or 藻類オイルが原則です。最終判断は必ず産科主治医にご相談ください。',
    },
    {
      q: '副作用・薬物相互作用・月コストは？',
      a: '両者とも安全性プロファイルは確立されていますが抗凝固薬併用と酸化管理が共通注意点です。\n\n月コスト比較：①藻類オイル＝Nordic Naturals Algae Omega 715mg/日 月¥4,500-5,500・Sports Research Vegan Omega-3 月¥3,500-4,500・Ovega-3 月¥4,000-5,500、②魚油オメガ3＝Nordic Naturals Ultimate Omega 2X 1,075mg/日 月¥1,700-2,500・iHerb 系コスパ製品 月¥800-2,000・処方薬 Vascepa（高純度 EPA）は健康保険適用で月¥3,000-5,000 程度（疾患制限あり）。\n\n副作用・併用注意：①抗凝固薬（ワルファリン・DOAC アピキサバン/リバーロキサバン）併用で出血リスクが報告され、手術前 1-2 週は両者とも休薬推奨、INR モニタリング必須、②高用量（>3g/日）で消化器症状・魚臭いげっぷ（藻類オイルは無臭が長所）・軟便、③酸化変性に弱く開封後は冷蔵推奨で酸化指標 TOTOX 値が低い製品を選ぶ、④糖尿病薬併用は血糖傾向変動で要モニタリング、⑤抗血小板薬・NSAIDs 連用は出血リスク加算、⑥妊娠中授乳中は DHA 推奨摂取量（200-300mg/日）の上限内で安全。\n\n化粧品メーカー視点では DHA→皮膚バリア脂質構成→経皮水分蒸散量改善の経路で algal oil・魚油どちらも意義あり、特に魚臭・水銀懸念で継続困難な人ほど藻類オイルで 6 ヶ月以上の継続性を確保する戦略が現実的です。',
    },
  ],
  // ── C2-Disc-β CSV 外発掘 5 件（5 FAQs each・C2 300 達成）2026-05-24 ────────────
  'gaba-vs-l-theanine': [
    {
      q: 'GABA と L-テアニンの違いは？',
      a: "作用機序・血液脳関門通過・主目的が完全に異なる補完関係です。\n\nGABA（γ-アミノ酪酸 100〜300mg/日）は中枢神経の主要抑制性神経伝達物質だが血液脳関門通過率は議論があり、サプリは腸-迷走神経経路で作用する仮説。Yamatsu 2014 J Nutr Sci Vitaminol RCT n=40 4 週間で 100mg 就寝前摂取により入眠潜時が有意短縮（p<0.05）・深睡眠増加、Abdou 2012 Biofactors RCT n=63 でストレス負荷後のα波増加・コルチゾール応答軽減を報告。\n\nL-テアニン（100〜400mg/日）は緑茶由来の遊離アミノ酸で血液脳関門通過確立、覚醒時のリラックス（α波増加・コルチゾール低下・グルタミン酸調節）に作用。Hidese 2021 Nutr Neurosci メタ解析 n=380 で L-テアニン（単独またはカフェイン併用）が注意・記憶・反応速度を有意改善・ストレス応答軽減、Kimura 2007 Biol Psychol RCT n=12 で急性ストレス負荷時の心拍変動・唾液コルチゾールをプラセボ比有意抑制を報告。「腸-迷走神経経路の即時リラックス（GABA）」vs「血液脳関門通過の日中集中＋リラックス（L-テアニン）」の対比と覚えると整理しやすい。",
    },
    {
